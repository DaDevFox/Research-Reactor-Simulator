...
public:
	...
	virtual Widget* findWidget(const Vector2i &p) override{
		Vector2i a = p - mPos + Vector2i(0, mScroll*getSizeDefect());
		for (auto it = mChildren.rbegin(); it != mChildren.rend(); ++it) {
			Widget *child = *it;
			if (child->visible() && child->contains(a))
				return child->findWidget(a);
		}
		return contains(p) ? this : nullptr;
	};
    virtual void draw(NVGcontext *ctx) override;
    virtual void save(Serializer &s) const override;
	virtual Vector2i absolutePosition() const override;
