...
public:
	...
	void setValue(const std::string &value, bool callback = true) { 
		if (mValue != value) {
			mValue = value;
			if (callback) { if (mCallback) mCallback(value); }
		}
	}
	...
	const std::string &currentValue() const { return mValueTemp; }
...
public:
    ...
    void setText(Scalar value) {
        Scalar clampedValue = std::min(std::max(value, mMinValue),mMaxValue);
        TextBox::setText(std::to_string(clampedValue));
    }
