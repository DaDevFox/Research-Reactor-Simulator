...
Screen::Screen(const Vector2i &size, const std::string &caption, bool resizable,
               bool fullscreen, bool windowsFrame, int colorBits, int alphaBits, int depthBits,
    ...
    : Widget(nullptr), mGLFWWindow(nullptr), mNVGContext(nullptr),
      ...
      mShutdownGLFWOnDestruct(false), mFullscreen(fullscreen), mWindowsFrame(windowsFrame) {
    memset(mCursors, 0, sizeof(GLFWcursor *) * (int) Cursor::CursorCount);
	...
	glfwWindowHint(GLFW_DECORATED, windowsFrame ? GL_TRUE : GL_FALSE);
