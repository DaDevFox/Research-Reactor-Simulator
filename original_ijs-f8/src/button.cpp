...
void Button::draw(NVGcontext *ctx) {
	...
	float halfBorder = mTheme->mBorderWidth / 2.f;

    nvgBeginPath(ctx);
    nvgStrokeWidth(ctx, mTheme->mBorderWidth);
    nvgRoundedRect(ctx, mPos.x() + halfBorder, mPos.y() + halfBorder + (mPushed ? 0.f : 0.5f), mSize.x() - 1,
                   mSize.y() - 1 - (mPushed ? 0.0f : mTheme->mBorderWidth), mTheme->mButtonCornerRadius);
    nvgStrokeColor(ctx, mTheme->mBorderLight);
    nvgStroke(ctx);

    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, mPos.x() + halfBorder, mPos.y() + halfBorder, mSize.x() - 1,
