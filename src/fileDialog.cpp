											...
											bool down, int /*modifiers*/)
{
	...
	if (itemClicked < 0 || (itemClicked > displayData.size() - 1))
		return false;
	if (down)
	{
		...
		if (lastClickFocus == itemClicked)
		{
			if (mDoubleClick)
			{
				...
				if (mSelectedChangedCallback)
					mSelectedChangedCallback(lastClickIndex);
			}
			else
			{
				mClickCallback(displayData[itemClicked].first);
...
void FileDialog::ItemList::draw(NVGcontext *ctx)
{
	...
	if (scissor[2] <= 0.f || scissor[3] <= 0.f) {
																	...
																	bounds =
	Vector4f(scissor[0], scissor[1], scissor[2], scissor[3]);
	}
	*/
	bool rule = true;
	for (int i = 0; i < 2; i++)
	...
	if (rule)
	{
		...
		if (mouseOnIndex >= displayData.size())
			mouseOnIndex = -1;
	}
	...
	: Window((Widget *)parent, title)
{
	...
	if (filetypes[0] != '*')
	{
		...
		if (lasti < fileTypeLen - 1)
			mTypes.push_back(filetypes.substr(lasti, fileTypeLen - lasti));
	}
	...
	baseLayout->appendRow(RelativeGridLayout::Size(
		mTheme->mWindowHeaderHeight,
		RelativeGridLayout::SizeType::Fixed)); // WINDOW HEADER
	baseLayout->appendRow(RelativeGridLayout::Size(
		TOOLBAR_HEIGHT, RelativeGridLayout::SizeType::Fixed));
	...
	mDirUpButton->setCallback([this]()
			...
			{
				if (prev[prev.length() - 1] == this->mDelimiter)
	...
	RelativeGridLayout::Anchor anchor_url = RelativeGridLayout::makeAnchor(
		1, 0, 1, 1, Alignment::Fill, Alignment::Middle);
	...
	RelativeGridLayout::Anchor anc1 = RelativeGridLayout::makeAnchor(
		0, mSave ? 1 : 0, 1, 1, Alignment::Minimum, Alignment::Middle);
	...
	RelativeGridLayout::Anchor anc2 = RelativeGridLayout::makeAnchor(
		1, mSave ? 1 : 0, 1, 1, Alignment::Maximum, Alignment::Middle);
...
void FileDialog::openDirectory(std::string directory, bool folders)
{
	...
	for (DirInfo a : find)
	{
		...
		else
		{
			...
			else
			{
				if (mTypes.size())
				{
					...
					{
						if (a.first.length() <= typ.length())
							continue;
						if (typ ==
							a.first.substr(a.first.length() - typ.length(), typ.length()))
						{
							endsWith = true;
							break;
						}
		...
		if (shouldAdd)
			mScrollArea->addItem(a);
	}
	this->prevDirectory = directory;
	mUrlBox->setValue(directory);
	...
	if (mSave)
	{
		...
		else
		{
			...
			if (name.empty() || name.length() > 37)
				return false; // check if name is too long or no name is selected
			for (int i = 0; i < name.length(); ++i)
			{ // check for invalid characters
				char one = name[i];
				if ((one < range1[0] || one > range1[1]) &&
					(one < range2[0] || one > range2[1]) &&
					(one < range3[0] || one > range3[1]))
				{
					invalid_found = true;
					for (int j = 0; j < 3; j++)
					{
			...
			if (strchr("._-", name[0]) != NULL)
				return false; // check if file name starts with invalid characters
			if (name[name.length() - 1] == '.')
				return false; // check if file name ends with dot
...
std::string FileDialog::getFileName()
{
	...
	if (mFolderGoal)
	{
		...
		{
			if (ret[ret.length() - 1] != mDelimiter)
				ret += mDelimiter;
