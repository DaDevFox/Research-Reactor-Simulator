...
CustomTabHeader::TabButton::TabButton(CustomTabHeader& header, const std::string& label)
...
void CustomTabHeader::TabButton::drawAtPosition(NVGcontext* ctx, const Vector2i& position, bool active)
{
	int xPos = position.x();
	int yPos = position.y();
	int width = mSize.x();
	...
	if (active)
	{
		...
		nvgBeginPath(ctx);
		nvgRoundedRect(ctx, xPos + halfBorder, yPos + halfBorder, width,
	...
	{
		textY += mSize.y() - textBounds[3] - mHeader->theme()->mTabButtonVerticalPadding;
	}
	...
	nvgBeginPath(ctx);
	nvgFillColor(ctx, active ? mHeader->activeTextColor() : mHeader->theme()->mTextColor);
...
void CustomTabHeader::TabButton::drawActiveBorderAt(NVGcontext* ctx, const Vector2i& position,
...
void CustomTabHeader::TabButton::drawInactiveBorderAt(NVGcontext* ctx, const Vector2i& position,
	float offset, const Color& color)
{
	int xPos = position.x();
	int yPos = position.y();
...
int CustomTabHeader::activeTab() const
{
	return mActiveTab;
}
...
void CustomTabHeader::ensureTabVisible(int index)
{
	...
	int allowedVisibleWidth = mSize.x() - 2 * (mStretch ? 0 : theme()->mTabControlWidth);
...
std::pair<Vector2i, Vector2i> CustomTabHeader::visibleButtonArea() const
{
	...
	Vector2i topLeft = (mStretch ? mPos : (mPos + Vector2i(theme()->mTabControlWidth, 0)));
	float width = std::accumulate(visibleBegin(), visibleEnd(), mStretch ? 0.f : theme()->mTabControlWidth,
...
std::pair<Vector2i, Vector2i> CustomTabHeader::activeButtonArea() const
{
	if (mVisibleStart == mVisibleEnd || mActiveTab < mVisibleStart || mActiveTab >= mVisibleEnd)
		return { Vector2i::Zero(), Vector2i::Zero() };
	float width = std::accumulate(visibleBegin(), activeIterator(), mStretch ? 0.f : theme()->mTabControlWidth,
		[](int acc, const TabButton& tb)
		{
			return acc + tb.size().x();
		});
...
Vector2i CustomTabHeader::preferredSize(NVGcontext* ctx) const
{
	...
	Vector2i size = mStretch ? Vector2i::Zero() : Vector2i(2 * theme()->mTabControlWidth, 0);
	for (TabButton tab : mTabButtons)
	{
		Vector2i tabPreferred = tab.preferredSize(ctx);
		if (tabPreferred.x() < theme()->mTabMinButtonWidth)
...
bool CustomTabHeader::mouseButtonEvent(const Vector2i& p, int button, bool down, int modifiers)
{
...
void CustomTabHeader::draw(NVGcontext* ctx)
{
	...
	Vector2i currentPosition = mPos + Vector2i(mStretch ? 0 : theme()->mTabControlWidth, 0);
