...
void SimulatorGUI::createOperationModesTab() {
  ...
  for (int i = 0; i < 3; i++) {
    ...
    layouts[i]->appendCol(RelativeGridLayout::Size(
        15.f, RelativeGridLayout::SizeType::Fixed));  // left border
    layouts[i]->appendCol(RelativeGridLayout::Size(
        250.f, RelativeGridLayout::SizeType::Fixed));  // settings
    layouts[i]->appendCol(1.f);                        // graph
    layouts[i]->appendCol(RelativeGridLayout::Size(
        15.f, RelativeGridLayout::SizeType::Fixed));  // right border
    layouts[i]->appendRow(RelativeGridLayout::Size(
    ...
    layouts[i]->appendRow(RelativeGridLayout::Size(
        20.f, RelativeGridLayout::SizeType::Fixed));  // bottom border
    tabs[i]->setLayout(layouts[i]);

    PeriodicalMode *use =
        reactor->regulatingRod()->getMode((SimulationModes)(i + 1));
    ...
    layouts[i]->setAnchor(graph, RelativeGridLayout::makeAnchor(
                                     2, 0, 2, simModeFields[i] + 3,
    ...
    operationModes[i]->setLimits(0., use->getPeriod(),
                                 -1.5 * use->getAmplitude(),
                                 1.5 * use->getAmplitude());
    ...
    // period
    periodBoxes[i] = makeSettingLabel<FloatBox<float>>(tabs[i], "Period: ", 100,
                                                       use->getPeriod());
    layouts[i]->setAnchor(
        periodBoxes[i]->parent(),
        RelativeGridLayout::makeAnchor(1, 1, 1, 1, Alignment::Minimum,
                                       Alignment::Middle));
    ...
    periodBoxes[i]->setCallback([use, i, this](float change) {
      ...
      use->fillXYaxis(operationModesPlots[0][i], operationModesPlots[1][i]);
      operationModes[i]->setLimits(0., change, -1.5 * use->getAmplitude(),
                                   1.5 * use->getAmplitude());
      operationModesTrackers[i]->setLimits(0., use->getPeriod(), 0., 1.);
    });

    // amplitude
    amplitudeBoxes[i] = makeSettingLabel<FloatBox<float>>(
        tabs[i], "Amplitude: ", 100, use->getAmplitude());
    layouts[i]->setAnchor(
        amplitudeBoxes[i]->parent(),
        RelativeGridLayout::makeAnchor(1, 2, 1, 1, Alignment::Minimum,
                                       Alignment::Middle));
    ...
    amplitudeBoxes[i]->setCallback([use, i, this](float change) {
      ...
      use->fillXYaxis(operationModesPlots[0][i], operationModesPlots[1][i]);
      operationModes[i]->setLimits(0., use->getPeriod(), -1.5 * change,
                                   1.5 * change);
    });
  ...
  for (int sqw = 0; sqw < 4; sqw++) {
    squareWaveBoxes[sqw] = makeSimulationSetting(
        tabs[0], (int)roundf(properties->squareWave.xIndex[sqw] * 100),
        sqw_settingNames[sqw]);
    layouts[0]->setAnchor(
        squareWaveBoxes[sqw]->parent(),
        RelativeGridLayout::makeAnchor(1, 3 + sqw, 1, 1, Alignment::Minimum,
                                       Alignment::Middle));
    squareWaveBoxes[sqw]->setCallback([this, sqw](int change) {
      ...
      if (sqw != 3) {
        ...
        if (squareWaveBoxes[sqw + 1]->value() < change)
          squareWaveBoxes[sqw + 1]->setValue(change);
      }
      reactor->regulatingRod()->squareWave()->fillXYaxis(
          operationModesPlots[0][0], operationModesPlots[1][0]);
    });
  }
  squareWaveSpeedBox =
      makeSettingLabel<SliderCheckBox>(tabs[0], "Use finite rod speed: ");
  layouts[0]->setAnchor(squareWaveSpeedBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 7, 1, 1, Alignment::Minimum, Alignment::Middle));
  squareWaveSpeedBox->setCallback([this](bool change) {
    properties->squareWaveUsesRodSpeed = change;
    if (change) {
      reactor->regulatingRod()->squareWave()->rodSpeed =
          reactor->regulatingRod()->getRodSpeed();
    } else {
  ...
  /* SINE MODE */
  sineModeBox =
      makeSettingLabel<ComboBox>(tabs[1], "Sine type: ", 0, sineModes);
  layouts[1]->setAnchor(sineModeBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 3, 1, 1, Alignment::Minimum, Alignment::Middle));
  ...
  sineModeBox->setCallback([this](int change) {
    ...
    reactor->regulatingRod()->sine()->fillXYaxis(operationModesPlots[0][1],
                                                 operationModesPlots[1][1]);
  });

  /* SAW TOOTH */
  for (int saw = 0; saw < 6; saw++) {
    sawToothBoxes[saw] = makeSimulationSetting(
        tabs[2], (int)roundf(properties->sawToothMode.xIndex[saw] * 100),
        saw_settingNames[saw]);
    layouts[2]->setAnchor(
        sawToothBoxes[saw]->parent(),
        RelativeGridLayout::makeAnchor(1, 3 + saw, 1, 1, Alignment::Minimum,
                                       Alignment::Middle));
    sawToothBoxes[saw]->setCallback([this, saw](int change) {
      ...
      if (saw < 5) {
        ...
        if (sawToothBoxes[saw + 1]->value() < change)
          sawToothBoxes[saw + 1]->setValue(change);
      }
      reactor->regulatingRod()->sawTooth()->fillXYaxis(
          operationModesPlots[0][2], operationModesPlots[1][2]);
    });
  }
  ...
  autoLayout->appendCol(RelativeGridLayout::Size(
      15.f, RelativeGridLayout::SizeType::Fixed));  // left border
  autoLayout->appendCol(1.f);                       // everything else
  autoLayout->appendRow(RelativeGridLayout::Size(
      15.f, RelativeGridLayout::SizeType::Fixed));  // top border
  for (int i = 0; i < 4; i++)
    autoLayout->appendRow(RelativeGridLayout::Size(
        30.f, RelativeGridLayout::SizeType::Fixed));  // 4 rows for 4 settings
  autoTab->setLayout(autoLayout);

  keepCurrentPowerBox =
      makeSettingLabel<SliderCheckBox>(autoTab, "Current power stability: ", 0);
  keepCurrentPowerBox->setChecked(properties->steadyCurrentPower);
  autoLayout->setAnchor(keepCurrentPowerBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 1, 1, 1, Alignment::Minimum, Alignment::Middle));

  steadyPowerBox = makeSettingLabel<FloatBox<double>>(
      autoTab, "Steady power: ", 100, properties->steadyGoalPower);
  ...
  autoLayout->setAnchor(steadyPowerBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 2, 1, 1, Alignment::Minimum, Alignment::Middle));
  ...
  avoidPeriodScramBox =
      makeSettingLabel<SliderCheckBox>(autoTab, "Avoid period SCRAM: ", 0);
  avoidPeriodScramBox->setChecked(reactor->getKeepCurrentPower());
  autoLayout->setAnchor(avoidPeriodScramBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 3, 1, 1, Alignment::Minimum, Alignment::Middle));
  ...
  automaticMarginBox = makeSettingLabel<FloatBox<float>>(
      autoTab, "Maximum power deviation: ", 0, properties->steadyMargin * 100);
  ...
  autoLayout->setAnchor(automaticMarginBox->parent(),
                        RelativeGridLayout::makeAnchor(
                            1, 4, 1, 1, Alignment::Minimum, Alignment::Middle));
