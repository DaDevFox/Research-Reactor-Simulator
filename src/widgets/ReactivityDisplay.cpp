...
void PeriodDisplay::draw(NVGcontext *ctx) {
  // Note: PeriodDisplay has no children, skip Widget::draw() to avoid scissor
  // clipping
  ...
  if (period > 0) {
    ...
    displayVal =
        std::min((1.f - posFromPeriod(-period)) * rangeYM, centerLocation);
  ...
  nvgRect(ctx, mPos.x() + mPadding + axesWidth,
          mPos.y() + verticalMargins * 0.98f, displayWidth, rangeY * 1.01f);
  ...
  nvgBeginPath(ctx);
  nvgRect(ctx, mPos.x() + mPadding + axesWidth,
          mPos.y() + verticalMargins + rangeYM, displayWidth, displayVal);
  nvgFillColor(ctx, Color(0, 255, 0, 255));
  nvgFill(ctx);

  // Main line for axes
  nvgBeginPath(ctx);
  nvgMoveTo(ctx, mPos.x() + mPadding + 0.8 * axesWidth,
            mPos.y() + verticalMargins);
  nvgLineTo(ctx, mPos.x() + mPadding + 0.8 * axesWidth,
            mPos.y() + verticalMargins + rangeY);
  nvgClosePath(ctx);
  ...
  /*
          ...
          nvgFillColor(ctx, Color(255, 255));
          nvgBeginPath(ctx);
          nvgMoveTo(ctx, mPos.x() + mPadding + i*(rodWidth + mSpacing) + 0.5f,
     mPos.y() + magnetPos[i] + verticalMargins); nvgLineTo(ctx, mPos.x() +
     mPadding + i*(rodWidth + mSpacing) + 0.5f - h, mPos.y() + magnetPos[i] +
     verticalMargins - pointerSize / 2.f); nvgLineTo(ctx, mPos.x() + mPadding +
     i*(rodWidth + mSpacing) + 0.5f - h, mPos.y() + magnetPos[i] +
     verticalMargins + pointerSize / 2.f); nvgClosePath(ctx); nvgFill(ctx);
  */
  nvgRestore(ctx);
}
