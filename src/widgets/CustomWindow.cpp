...
void CustomWindow::draw(NVGcontext *ctx) {
  ...
  if (!mTitle.empty()) {
    ...
    nvgText(ctx, mPos.x() + mSize.x() / 2.f, mPos.y() + hh / 2.f,
  ...
  nvgSave(ctx);
  nvgIntersectScissor(
      ctx, mPos.x() + mBorderThickness / 2.f, mPos.y() + mBorderThickness / 2.f,
      mSize.x() - mBorderThickness, mSize.y() - mBorderThickness);
...
bool CustomWindow::mouseButtonEvent(const Vector2i &p, int button, bool down,
                                    int modifiers) {
  if (!mEnabled) return false;
...
bool CustomWindow::mouseMotionEvent(const Vector2i &p, const Vector2i &rel,
                                    int button, int modifiers) {
  if (mEnabled) {
...
void CustomWindow::setBorder(bool value) { mBorder = value; }

bool CustomWindow::border() { return mBorder; }

void CustomWindow::setBorderColor(Color value) { mBorderColor = value; }

Color CustomWindow::borderColor() { return mBorderColor; }

void CustomWindow::setBorderThickness(float value) { mBorderThickness = value; }

float CustomWindow::borderThickness() { return mBorderThickness; }
...
void CustomWindow::setTheme(CustomTheme *theme) {
  if (mCustomTheme.get() == theme) return;
  mCustomTheme = theme;
