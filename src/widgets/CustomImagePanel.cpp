...
bool CustomImagePanel::mouseMotionEvent(const Vector2i &p,
                                        const Vector2i & /* rel */,
...
bool CustomImagePanel::mouseButtonEvent(const Vector2i &p, int /* button */,
                                        bool down, int /* modifiers */) {
  int index = indexForPosition(p);
  if (index >= 0 && mCallback && down) mCallback(index);
  return true;
}
...
void CustomImagePanel::draw(NVGcontext *ctx) {
  ...
  for (size_t i = 0; i < mImages.size(); ++i) {
    if (mImages[i].first == 0 || mImages[i].first == -1) {
      fprintf(stderr,
              "[CustomImagePanel] Warning: Invalid image handle (%d) for image "
              "%zu: %s\n",
    ...
    NVGpaint imgPaint =
        nvgImagePattern(ctx, p.x() + ix, p.y() + iy, iw, ih, 0,
    ...
    NVGpaint shadowPaint =
        nvgBoxGradient(ctx, p.x() - 1, p.y(), mThumbSize + 2, mThumbSize + 2, 5,
                       3, nvgRGBA(0, 0, 0, 128), nvgRGBA(0, 0, 0, 0));
    nvgBeginPath(ctx);
