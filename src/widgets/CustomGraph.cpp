...
CustomGraph::~CustomGraph() { delete[] graphs; }
...
void CustomGraph::draw(NVGcontext *ctx) {
  ...
  nvgBeginPath(ctx);
  nvgRect(ctx, mPos.x() + padding[0], mPos.y() + padding[1], graphRangeX,
          graphRangeY);
    ...
    if (currentElement->getEnabled()) {
      ...
      if (currentElement->graphType() == GraphElement::GraphType::PlotDeque) {
        ...
        if (current->getPlotRange() > 0) {
          ...
          nvgLineTo(ctx, xPos + graphRangeX * current->getXat(plotStartIndex),
                    y1);

          switch (current->getDrawMode()) {
            case DrawMode::Default: {
              double vx, vy;
              for (size_t i = plotStartIndex + 1;
                   i <= (size_t)current->getPlotEnd(); i++) {
            ...
            case DrawMode::Smart: {
              ...
              double step = (double)current->getPlotRange() /
                            (pixels * current->getPixelDrawRatio());
              ...
              for (size_t i = 1; i < pixels; i++) {
                ...
                if (step >= 1.) {
                  ...
                  nvgLineTo(ctx, xPos + graphRangeX * current->getXat(rounda),
                            yPos + (1 - current->getYat(rounda)) * graphRangeY);
                } else {
                  ceila = std::min(cap, (size_t)ceil(a));
                  floora = (size_t)floor(a);
                  if (ceila == floora) {
                    nvgLineTo(
                        ctx, xPos + graphRangeX * current->getXat(floora),
                        yPos + (1 - current->getYat(floora)) * graphRangeY);
                  } else {
                    // Linear interpolation
                    vx = current->getXat(floora) * (ceila - a) +
                         current->getXat(ceila) * (a - floora);
                    vy = current->getYat(floora) * (ceila - a) +
                         current->getYat(ceila) * (a - floora);
              ...
              float yLast =
                  yPos +
                  (1 - current->getYat(current->getPlotEnd())) * graphRangeY;
              // Draw last line
            ...
            case DrawMode::SuperSmart: {
              ...
              for (double a = (double)plotStartIndex + 1;
                   a <= (double)current->getPlotEnd(); a += adaptiveStep) {
                // Linear fit
                ...
                double thisValueX;
                if (ceila == floora) {
                  // If the ceiling and floor of "a" are the same, use one of
                  // them
                  thisValueY = current->getYat(floora);
                  thisValueX = current->getXat(floora);
                  nvgLineTo(ctx, xPos + graphRangeX * thisValueX,
                            yPos + (1 - thisValueY) * graphRangeY);
                } else {
                  // If the ceiling and floor of "a" are not equal, linear
                  // interpolate
                  thisValueX = current->getXat(floora) * (ceila - a) +
                               current->getXat(ceila) * (a - floora);
                  thisValueY = current->getYat(floora) * (ceila - a) +
                               current->getYat(ceila) * (a - floora);
                  nvgLineTo(ctx, thisValueX * graphRangeX + xPos,
                            (1 - thisValueY) * graphRangeY + yPos);
                }
                // k factor of a line connecting the two points
                double deviation = std::max(
                    abs(thisValueY - lastValueY) / (thisValueX - lastValueX),
                    0.1);
                // Update last values
      ...
      } else if (currentElement->graphType() ==
                 GraphElement::GraphType::Bezier) {
        BezierCurve *current = (BezierCurve *)currentElement;
        nvgBeginPath(ctx);
        nvgMoveTo(ctx, xPos, yPos + graphRangeY);
        nvgBezierTo(ctx, xPos + current->getParameter(0) * graphRangeX,
                    yPos + (1 - current->getParameter(1)) * graphRangeY,
                    xPos + current->getParameter(2) * graphRangeX,
                    yPos + (1 - current->getParameter(3)) * graphRangeY,
                    xPos + graphRangeX, yPos);
      ...
      if (currentElement->isFill()) {
        ...
        nvgLineTo(ctx, xPos + graphRangeX + halfDraw,
                  yPos + graphRangeY + halfDraw);
        // To point (-s/2, -s/2)
        ...
        if (relFill >= 0.f) {
          ...
          if (currentElement->graphType() == GraphElement::GraphType::Bezier) {
            // Overflow part
            nvgIntersectScissor(
                ctx, xPos, yPos, relFill * graphRangeX,
                (1 - currentElement->getPointerPosition()) * graphRangeY);
            // Fill
            ...
            nvgIntersectScissor(
                ctx, xPos,
                yPos + (1 - currentElement->getPointerPosition()) * graphRangeY,
                relFill * graphRangeX,
                currentElement->getPointerPosition() * graphRangeY);
          } else {
            // Other plot elements
            nvgIntersectScissor(ctx, xPos, yPos, relFill * graphRangeX,
                                graphRangeY);
          }
          // Fill
      ...
      int direction = 1;
      if (currentElement->getAxisPosition() ==
          GraphElement::AxisLocation::Right) {
      ...
      if (currentElement->getAxisShown()) {
        ...
        nvgMoveTo(ctx, new_xPos, yPos);
        nvgLineTo(ctx,
                  new_xPos - direction * currentElement->getMajorTickSize(),
                  yPos);
        if (currentElement->getTextShown()) {
          string content =
              currentElement->getOverridenLimits()[3].length()
                  ? currentElement->getOverridenLimits()[3]
                  : currentElement->formatNumber(
                        limits[3] * currentElement->getLimitMultiplier());
          nvgFontSize(ctx, currentElement->getMainTickFontSize());
          nvgText(
              ctx,
              new_xPos - direction * (currentElement->getMajorTickSize() + 3),
              yPos, content.c_str(), NULL);
        }
        if (currentElement->getYlog()) {
          for (double i = floor(limLog[2]) + 1; i < ceil(limLog[3]); i++) {
            float thisY =
                yPos + graphRangeY *
                           (1. - ((i - limLog[2]) / (limLog[3] - limLog[2])));
            ...
            if (currentElement->getTextShown()) {
              ...
              nvgText(ctx,
                      new_xPos -
                          direction * (currentElement->getMajorTickSize() + 3),
                      thisY, ("1e" + std::to_string((int)i)).c_str(), NULL);
            }
          }
        } else {
          if (currentElement->getMajorTickNumber() != 0) {
            float tickDiff =
                graphRangeY / (currentElement->getMajorTickNumber() + 1);
              ...
              if (currentElement->getTextShown()) {
                ...
                nvgText(
                    ...
                    currentElement
                        ->formatNumber(
                            (limits[2] +
                             (currentElement->getMajorTickNumber() + 1 - i) *
                                 (limits[3] - limits[2]) /
                                 (currentElement->getMajorTickNumber() + 1)) *
                            currentElement->getLimitMultiplier())
                        .c_str(),
                    NULL);
              }
            }
        ...
        if (currentElement->getTextShown()) {
          string content =
              currentElement->getOverridenLimits()[2].length()
                  ? currentElement->getOverridenLimits()[2]
                  : currentElement->formatNumber(
                        limits[2] * currentElement->getLimitMultiplier());
          nvgFontSize(ctx, currentElement->getMainTickFontSize());
          nvgText(
              ctx,
              new_xPos - direction * (currentElement->getMajorTickSize() + 3),
              yPos + graphRangeY, content.c_str(), NULL);
        }
        nvgStroke(ctx);
        ...
        if (currentElement->getYlog()) {
          ...
          for (double i = floor(limLog[2]); i < upperLim; i++) {
            for (int step = 2; step < 10; step++) {
              ...
              if (holder > limits[2] && holder < limits[3]) {
                yHolder =
                    yPos + graphRangeY * (1. - ((log10(holder) - limLog[2]) /
                                                (limLog[3] - limLog[2])));
                nvgMoveTo(ctx, new_xPos, yHolder);
                nvgLineTo(
                    ctx,
                    new_xPos - direction * currentElement->getMinorTickSize(),
                    yHolder);
              } else {
                continue;
              }
          ...
          if (currentElement->getMinorTickNumber() != 0) {
            ...
            float tickDiffMinor =
                graphRangeY / ((currentElement->getMajorTickNumber() + 1) *
                               (currentElement->getMinorTickNumber() + 1));
            nvgStrokeWidth(ctx, currentElement->getMinorTickWidth());
            nvgBeginPath(ctx);
            for (size_t major = 0;
                 major < currentElement->getMajorTickNumber() + 1; major++) {
              for (size_t i = 1; i <= currentElement->getMinorTickNumber();
                   i++) {
      ...
      /* HORIZONTAL AXIS */
      float new_yPos =
          yPos + graphRangeY + currentElement->getHorizontalAxisOffset();
      direction = 1;
      if (currentElement->getHorizontalAxisPosition() ==
          GraphElement::HorizontalAxisLocation::Top) {
      ...
      if (currentElement->getHorizontalAxisShown()) {
        ...
        nvgMoveTo(ctx, xPos, new_yPos);
        nvgLineTo(ctx, xPos,
                  new_yPos + direction * currentElement->getMajorTickSize());
        if (currentElement->getTextShown()) {
          string content =
              currentElement->getOverridenLimits()[0].length()
                  ? currentElement->getOverridenLimits()[0]
                  : currentElement->formatNumber(
                        limits[0] * currentElement->getLimitMultiplier());
          nvgFontSize(ctx, currentElement->getMainTickFontSize());
          nvgText(
              ctx, xPos,
              new_yPos + direction * (currentElement->getMajorTickSize() + 3),
              content.c_str(), NULL);
        }
        if (currentElement->getHorizontalMajorTickNumber() != 0) {
          float tickDiff = graphRangeX /
                           (currentElement->getHorizontalMajorTickNumber() + 1);
          for (size_t i = 1;
               i <= currentElement->getHorizontalMajorTickNumber(); i++) {
            ...
            if (currentElement->getTextShown()) {
              ...
              nvgText(
                  ...
                  currentElement
                      ->formatNumber(
                          (currentElement->limits()[0] +
                           i * (limits[1] - limits[0]) /
                               (currentElement->getHorizontalMajorTickNumber() +
                                1)) *
                          currentElement->getLimitHorizontalMultiplier())
                      .c_str(),
                  NULL);
            }
          }
        ...
        nvgLineTo(ctx, xPos + graphRangeX,
                  new_yPos + direction * currentElement->getMajorTickSize());
        if (currentElement->getTextShown()) {
          string content =
              currentElement->getOverridenLimits()[1].length()
                  ? currentElement->getOverridenLimits()[1]
                  : currentElement->formatNumber(
                        limits[1] *
                        currentElement->getLimitHorizontalMultiplier());
          nvgFontSize(ctx, currentElement->getMainTickFontSize());
          nvgText(
              ctx, xPos + graphRangeX,
              new_yPos + direction * (currentElement->getMajorTickSize() + 3),
              content.c_str(), NULL);
        }
        nvgStroke(ctx);
        ...
        if (currentElement->getHorizontalMinorTickNumber() != 0) {
          ...
          float tickDiffMinor =
              graphRangeX /
              ((currentElement->getHorizontalMajorTickNumber() + 1) *
               (currentElement->getHorizontalMinorTickNumber() + 1));
          nvgStrokeWidth(ctx, currentElement->getMinorTickWidth());
          nvgBeginPath(ctx);
          for (size_t major = 0;
               major < currentElement->getHorizontalMajorTickNumber() + 1;
               major++) {
            for (size_t i = 1;
                 i <= currentElement->getHorizontalMinorTickNumber(); i++) {
              float thisX = xPos + major * tickDiffMajor + i * tickDiffMinor;
              nvgMoveTo(ctx, thisX, new_yPos);
              nvgLineTo(
                  ctx, thisX,
                  new_yPos + direction * currentElement->getMinorTickSize());
            }
          }
          nvgStroke(ctx);
        }
        ...
        if (currentElement->getTextShown()) {
          ...
          nvgText(ctx, xPos + graphRangeX / 2.f,
                  new_yPos +
                      direction * (currentElement->getMajorTickSize() +
                                   currentElement->getHorizontalTextOffset()),
                  content.c_str(), NULL);
        }
      }
    }
  ...
  for (size_t index = 0; index < mActualGraphNumber; index++) {
    ...
    if (current->getEnabled()) {
      ...
      if (current->getPointerShown()) {
        ...
        if (current->getPointerOverride()) {
          ...
          if (current->graphType() == 1) {
            finalY = (float)(((Plot *)current)
                                 ->getYat(((Plot *)current)->getPlotEnd()));
          } else {
            finalY = 0.f;
          }
        }
        ...
        float triMove =
            direction * (float)sqrt(3) * current->getPointerSize() / 2.f;
        if (finalY < 0.f) {
          nvgMoveTo(ctx, new_xPos - direction * pointerSize / 2.f,
                    yPos + graphRangeY + abs(triMove) +
                        current->getMajorTickWidth() / 2.f);
          nvgLineTo(ctx, new_xPos,
                    yPos + graphRangeY + current->getMajorTickWidth() / 2.f);
          nvgLineTo(ctx, new_xPos - direction * pointerSize,
                    yPos + graphRangeY + current->getMajorTickWidth() / 2.f);
        } else if (finalY > 1.f) {
          nvgMoveTo(ctx, new_xPos - direction * pointerSize / 2.f,
                    yPos - abs(triMove) - current->getMajorTickWidth() / 2.f);
          nvgLineTo(ctx, new_xPos, yPos - current->getMajorTickWidth() / 2.f);
          nvgLineTo(ctx, new_xPos - direction * pointerSize,
                    yPos - current->getMajorTickWidth() / 2.f);
        } else {
      ...
      if (current->getHorizontalPointerShown()) {
        ...
        float new_yPos =
            yPos + graphRangeY + current->getHorizontalAxisOffset();
        int direction = 1;
        if (current->getHorizontalAxisPosition() ==
            GraphElement::HorizontalAxisLocation::Top) {
        ...
        if (finalX < 0.f) {
          ...
          nvgLineTo(
              ctx, xPos - abs(triMove) - current->getMajorTickWidth() / 2.f,
              new_yPos + direction * current->getHorizontalPointerSize() / 2.f);
          nvgLineTo(ctx, xPos - current->getMajorTickWidth() / 2.f,
                    new_yPos + direction * current->getHorizontalPointerSize());
        } else if (finalX > 1.f) {
          nvgMoveTo(ctx,
                    xPos + graphRangeX + current->getMajorTickWidth() / 2.f,
                    new_yPos);
          nvgLineTo(
              ctx,
              xPos + graphRangeX + abs(triMove) +
                  current->getMajorTickWidth() / 2.f,
              new_yPos + direction * current->getHorizontalPointerSize() / 2.f);
          nvgLineTo(ctx,
                    xPos + graphRangeX + current->getMajorTickWidth() / 2.f,
          ...
          nvgLineTo(ctx, finalX + direction * pointerSize / 2.f,
                    new_yPos + triMove);
          nvgLineTo(ctx, finalX - direction * pointerSize / 2.f,
                    new_yPos + triMove);
        }
  ...
  for (size_t plotIndex = 0; plotIndex < (size_t)barGraphNumber_; plotIndex++) {
    ...
    for (size_t i = range[0]; i <= range[1]; i++) {
      ...
      nvgBeginPath(ctx);
      nvgRect(ctx, xPos + (2 * (i - range[0]) + 1) * spacing,
              yPos + (1 - current->getY(i)) * graphRangeY, spacing,
              current->getY(i) * graphRangeY);
  ...
  if (!mFooter.empty()) {
    ...
    nvgFillColor(ctx, mTextColor);
    nvgText(ctx, mPos.x() + mSize.x() - 3, mPos.y() + mSize.y() - 1,
            mFooter.c_str(), NULL);
  }
...
void CustomGraph::setPadding(float left, float top, float right, float bottom) {
  padding[0] = left;
  padding[1] = top;
  padding[2] = right;
  padding[3] = bottom;
...
BarGraph *CustomGraph::getBarGraph(size_t index) { return barPlots[index]; }
