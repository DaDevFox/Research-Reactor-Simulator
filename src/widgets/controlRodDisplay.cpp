...
void ControlRodDisplay::draw(NVGcontext *ctx) {
  // Note: ControlRodDisplay has no children, skip Widget::draw() to avoid
  // scissor clipping
  ...
  for (int i = 0; i < 3; i++) {
    ...
    nvgRect(ctx, mPos.x() + (rodWidth + rodSpacing) * i, mPos.y(), rodWidth,
            mSize.y());
  }
  ...
  for (int i = 0; i < 3; i++) {
    ...
    if (relPosRod > 0.f) {
      for (int m = 0; m < 2; m++) {
        upper[m] = nvgLinearGradient(
            ctx, x[i] + rodWgrad, 0,          // startx, starty
            m ? (x[i] + rodWidth) : x[i], 0,  // endx, endy
            rodEnabled[i] ? mRodExtrudedWhiteColor : mRodDisabledWhiteColor,
            rodEnabled[i] ? mRodExtrudedColor
                          : mRodDisabledColor);  // startcol, endcol
        nvgBeginPath(ctx);
        nvgRect(ctx, x[i] + rodBorder + (m ? rodWgrad : 0.f), mPos.y(),
                (2 - m) * rodWgrad, relPosRod);
    ...
    if (relPosRod < rodSize) {
      nvgBeginPath(ctx);
      nvgRect(ctx, x[i] + rodBorder, mPos.y() + relPosRod,
              rodWidth - 2 * rodBorder, rodSize - relPosRod);
    ...
    nvgLineTo(ctx, x[i] + rodWidth + h,
              mPos.y() + relPosMagnet - pointerSize / 2.f);
    nvgLineTo(ctx, x[i] + rodWidth + h,
              mPos.y() + relPosMagnet + pointerSize / 2.f);
    nvgClosePath(ctx);
    nvgFill(ctx);
