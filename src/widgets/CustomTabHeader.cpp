...
CustomTabHeader::TabButton::TabButton(CustomTabHeader& header,
                                      const std::string& label)
    : mHeader(&header), mLabel(label) {}

Vector2i CustomTabHeader::TabButton::preferredSize(NVGcontext* ctx) const {
  ...
  int buttonHeight =
      bounds[3] - bounds[1] + 2 * mHeader->theme()->mTabButtonVerticalPadding;
  return Vector2i(buttonWidth, buttonHeight);
}

void CustomTabHeader::TabButton::calculateVisibleString(NVGcontext* ctx) {
  ...
  if (displayedText.next[0]) {
    auto truncatedWidth = nvgTextBounds(ctx, 0.0f, 0.0f, displayedText.start,
                                        displayedText.end, nullptr);
    auto dotsWidth = nvgTextBounds(ctx, 0.0f, 0.0f, dots, nullptr, nullptr);
    while ((truncatedWidth + dotsWidth +
            mHeader->theme()->mTabButtonHorizontalPadding) > mSize.x() &&
           displayedText.end != displayedText.start) {
      --displayedText.end;
      truncatedWidth = nvgTextBounds(ctx, 0.0f, 0.0f, displayedText.start,
                                     displayedText.end, nullptr);
    }
...
void CustomTabHeader::TabButton::drawAtPosition(NVGcontext* ctx,
  ...
  nvgFillPaint(ctx, nvgLinearGradient(ctx, xPos, yPos, xPos, yPos + height,
                                      gradTop, gradBot));
  nvgFill(ctx);
  ...
  if (active) {
    ...
    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, xPos + halfBorder, yPos + halfBorder, width, height + 1,
                   theme->mButtonCornerRadius);
  ...
  float textBounds[4];
  nvgTextBounds(ctx, 0.0f, 0.0f, mLabel.c_str(), nullptr, textBounds);
  int textX = xPos;

  if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_LEFT) {
    textX += mHeader->theme()->mTabButtonHorizontalPadding;
  } else if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_CENTER) {
    textX += (int)std::roundf((mSize.x() - textBounds[2]) / 2.f);
  } else if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_RIGHT) {
    textX += (int)std::roundf(mSize.x() - textBounds[2]);
  }
  int textY = yPos;
  if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_BOTTOM) {
    textY += mSize.y() - textBounds[3];
  } else if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_BASELINE) {
    textY +=
        mSize.y() - textBounds[3] - mHeader->theme()->mTabButtonVerticalPadding;
  } else if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_MIDDLE) {
    textY += (int)std::roundf((mSize.y() - textBounds[3]) / 2.f);
  } else if (mHeader->mButtonAlignment & NVGalign::NVG_ALIGN_TOP) {
    textY += mHeader->theme()->mTabButtonVerticalPadding;
  }

  nvgBeginPath(ctx);
  nvgFillColor(
      ctx, active ? mHeader->activeTextColor() : mHeader->theme()->mTextColor);
...
void CustomTabHeader::TabButton::drawActiveBorderAt(NVGcontext* ctx,
                                                    const Vector2i& position,
                                                    float offset,
                                                    const Color& color) {
  int xPos = position.x();
...
void CustomTabHeader::TabButton::drawInactiveBorderAt(NVGcontext* ctx,
                                                      const Vector2i& position,
                                                      float offset,
                                                      const Color& color) {
  int xPos = position.x();
  ...
  nvgRoundedRect(ctx, xPos + offset, yPos + offset, width - offset,
                 height - offset, mHeader->theme()->mButtonCornerRadius);
  nvgStrokeColor(ctx, color);
  nvgStroke(ctx);
}
...
int CustomTabHeader::activeTab() const { return mActiveTab; }

bool CustomTabHeader::isTabVisible(int index) const {
  return index >= mVisibleStart && index < mVisibleEnd;
}

void CustomTabHeader::addTab(const std::string& label) {
  addTab(tabCount(), label);
}

void CustomTabHeader::addTab(int index, const std::string& label) {
...
int CustomTabHeader::removeTab(const std::string& label) {
  auto element =
...
void CustomTabHeader::removeTab(int index) {
  assert(index < tabCount());
  mTabButtons.erase(std::next(mTabButtons.begin(), index));
  if (index == mActiveTab && index != 0) setActiveTab(index - 1);
}

const std::string& CustomTabHeader::tabLabelAt(int index) const {
  assert(index < tabCount());
  return mTabButtons[index].label();
}

int CustomTabHeader::tabIndex(const std::string& label) {
  auto it =
      std::find_if(mTabButtons.begin(), mTabButtons.end(),
                   [&](const TabButton& tb) { return label == tb.label(); });
...
void CustomTabHeader::ensureTabVisible(int index) {
  auto visibleArea = visibleButtonArea();
  auto visibleWidth = visibleArea.second.x() - visibleArea.first.x();
  int allowedVisibleWidth =
      mSize.x() - 2 * (mStretch ? 0 : theme()->mTabControlWidth);
...
std::pair<Vector2i, Vector2i> CustomTabHeader::visibleButtonArea() const {
  if (mVisibleStart == mVisibleEnd) return {Vector2i::Zero(), Vector2i::Zero()};
  Vector2i topLeft =
      (mStretch ? mPos : (mPos + Vector2i(theme()->mTabControlWidth, 0)));
  float width = std::accumulate(
      visibleBegin(), visibleEnd(), mStretch ? 0.f : theme()->mTabControlWidth,
      [](int acc, const TabButton& tb) { return acc + tb.size().x(); });
  Vector2i bottomRight = mPos + Vector2i(width, mSize.y());
  return {topLeft, bottomRight};
}

std::pair<Vector2i, Vector2i> CustomTabHeader::activeButtonArea() const {
  if (mVisibleStart == mVisibleEnd || mActiveTab < mVisibleStart ||
      mActiveTab >= mVisibleEnd)
    return {Vector2i::Zero(), Vector2i::Zero()};
  float width = std::accumulate(
      visibleBegin(), activeIterator(),
      mStretch ? 0.f : theme()->mTabControlWidth,
      [](int acc, const TabButton& tb) { return acc + tb.size().x(); });
...
void CustomTabHeader::performLayout(NVGcontext* ctx) {
  Widget::performLayout(ctx);

  if (mStretch) {
    int w = (int)std::floor((double)mParent->size().x() / tabCount());
    for (TabButton& tab : mTabButtons) {
      Vector2i tabPreferred = tab.preferredSize(ctx);
      tab.setSize(Vector2i(w, tabPreferred.y()));
...
Vector2i CustomTabHeader::preferredSize(NVGcontext* ctx) const {
  ...
  Vector2i size =
      mStretch ? Vector2i::Zero() : Vector2i(2 * theme()->mTabControlWidth, 0);
  for (TabButton tab : mTabButtons) {
    Vector2i tabPreferred = tab.preferredSize(ctx);
    if (tabPreferred.x() < theme()->mTabMinButtonWidth)
      tabPreferred.x() = theme()->mTabMinButtonWidth;
  ...
  if (mStretch) size[0] = mParent->size()[0];
  return size;
}

bool CustomTabHeader::mouseButtonEvent(const Vector2i& p, int button, bool down,
                                       int modifiers) {
  ...
  if (button == GLFW_MOUSE_BUTTON_1 && down) {
    switch (locateClick(p)) {
      ...
      case ClickLocation::TabButtons:
        auto first = visibleBegin();
        auto last = visibleEnd();
        int currentPosition = mStretch ? 0 : theme()->mTabControlWidth;
        int endPosition = p.x();
...
void CustomTabHeader::draw(NVGcontext* ctx) {
  ...
  if (mOverflowing) drawControls(ctx);
  ...
  Vector2i currentPosition =
      mPos + Vector2i(mStretch ? 0 : theme()->mTabControlWidth, 0);
  ...
  if (drawActive) active->drawAtPosition(ctx, activePosition, true);
}

void CustomTabHeader::calculateVisibleEnd() {
  auto first = visibleBegin();
  auto last = mTabButtons.end();
  int currentPosition = mStretch ? 0 : theme()->mTabControlWidth;
  int lastPosition = mSize.x() - currentPosition;
  auto firstInvisible = std::find_if(
...
void CustomTabHeader::drawControls(NVGcontext* ctx) {
  // Left button.
  ...
  Vector2f leftIconPos =
      mPos.cast<float>() + Vector2f(xScaleLeft * theme()->mTabControlWidth,
                                    yScaleLeft * mSize.cast<float>().y());
  ...
  auto leftControlsPos =
      mPos.cast<float>() +
      Vector2f(mSize.cast<float>().x() - theme()->mTabControlWidth, 0);
  Vector2f rightIconPos =
      leftControlsPos + Vector2f(xScaleRight * theme()->mTabControlWidth,
                                 yScaleRight * mSize.cast<float>().y());
  nvgText(ctx, rightIconPos.x(), rightIconPos.y() + 1, iconRight.data(),
          nullptr);
}

CustomTabHeader::ClickLocation CustomTabHeader::locateClick(const Vector2i& p) {
...
void CustomTabHeader::onArrowLeft() {
  if (mVisibleStart == 0) return;
  --mVisibleStart;
  calculateVisibleEnd();
}

void CustomTabHeader::onArrowRight() {
