...
CustomLabel::CustomLabel(Widget* parent, const std::string& caption,
...
void CustomLabel::setTheme(Theme* theme) {
...
Vector2i CustomLabel::preferredSize(NVGcontext* ctx) const {
  ...
  for (int i = 0; i < 2; i++) {
    retSize[i] = mFixedSize[i] ? mFixedSize[i]
                               : bounds[2 + i] - bounds[i] + mPadding[i] +
                                     mPadding[i + 2];
  }
  return retSize;
}

void CustomLabel::draw(NVGcontext* ctx) {
  if (mGlow) {
    nvgBeginPath(ctx);
    NVGpaint grad = nvgBoxGradient(
        ctx, mPos.x(), mPos.y(), mSize.x(), mSize.y(), mRoundedEdges, mFeather,
        mGlowColor, Color(mGlowColor.r(), mGlowColor.g(), mGlowColor.b(), 0.f));
    nvgFillPaint(ctx, grad);
    nvgRect(ctx, mPos.x() - mFeather, mPos.y() - mFeather,
            mSize.x() + 2.f * mFeather, mSize.y() + 2.f * mFeather);
    nvgFill(ctx);
  }
  ...
  Vector2i useSize = Vector2i(mSize.x() - mPadding[0] - mPadding[2],
                              mSize.y() - mPadding[1] - mPadding[3]);
  ...
  nvgText(ctx, mPos.x() + offset[0], mPos.y() + offset[1], mCaption.c_str(),
          nullptr);
}

void CustomLabel::save(Serializer& s) const {
  Widget::save(s);
...
bool CustomLabel::load(Serializer& s) {
