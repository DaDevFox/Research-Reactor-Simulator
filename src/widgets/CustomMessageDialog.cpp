...
CustomMessageDialog::CustomMessageDialog(Widget* parent, Type type,
                                         ...
                                         bool altButton)
    : Window(parent, title) {
  setLayout(new BoxLayout(Orientation::Vertical, Alignment::Fill, 10, 0));
  setModal(true);
  ...
  switch (type) {
    case Type::Information:
      icon = ENTYPO_ICON_INFO_WITH_CIRCLE;
      break;
    case Type::Question:
      icon = ENTYPO_ICON_HELP_WITH_CIRCLE;
      break;
    case Type::Warning:
  ...
  CustomLabel* iconLabel =
      new CustomLabel(panel1, std::string(utf8(icon).data()), "icons");
  iconLabel->setFontSize(50);
  mMessageLabel = new CustomLabel(panel1, message);
  mMessageLabel->setTextAlignment(CustomLabel::TextAlign::LEFT ||
  ...
  if (altButton) {
    relLayout->appendCol(1.f);
    Button* button =
        new Button(panel2, altButtonText, ENTYPO_ICON_CIRCLE_WITH_CROSS);
    relLayout->setAnchor(
        button, RelativeGridLayout::makeAnchor(1, 0, 1, 1, Alignment::Middle,
                                               Alignment::Middle));
    button->setCallback([&] {
  ...
  relLayout->setAnchor(
      button, RelativeGridLayout::makeAnchor(0, 0, 1, 1, Alignment::Middle,
                                             Alignment::Middle));
  button->setCallback([&] {
