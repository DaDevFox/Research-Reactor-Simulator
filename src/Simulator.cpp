...
std::string formatDecimalsDoubleWithTrailing(const double x,
                                             const int decDigits,
...
void Simulator::dataToFile(std::string fileName) {
  ...
  logFile << "#                  Research reactor simulator log " << s
          << "                   #\n";
  logFile << "#Time[h:m:s:ms] Reactivity[pcm] Inserted-reactivity[pcm] "
             "Power[W] Temp[C] Xenon-conc.[g/m3] Iodine-conc.[g/m3]#\n";
  logFile << "#################################################################"
             "##############################################\n";
  ...
  for (long shift = 0L; shift < len / data_division; shift++) {
    ...
    logFile << formatTime(time_[idx]) << "\t" << reactivity_[idx] << "\t"
            << rodReactivity_[idx] << "\t"
            << powerFromNeutrons(state_vector_[0][idx]) << "\t"
            << temperature_[idx] << "\t" << xenon_[poisonIdx] << "\t"
            << iodine_[poisonIdx] << std::endl;
  }
  logFile.close();
}
...
void Simulator::rodsToFile(std::string fileName) {
  ...
  rodFile << "#################################################################"
             "####\n";
  rodFile << "# Pos(0-1) | Safety rod[pcm] | Pulse rod[pcm] | Regulating "
             "rod[pcm] #\n";
  rodFile << "#################################################################"
             "####\n";
    ...
    for (size_t j = 0; j < NUMBER_OF_CONTROL_RODS; j++) {
      if (i <= *rods[j]->getRodSteps()) {
        ...
        if (i <= *rods[j]->getRodSteps() - 1) {
          rodFile << std::setprecision(2) << fixed
                  << rods[j]->getPCMat((float)(i + 1)) -
                         rods[j]->getPCMat((float)i)
                  << "\t";
        } else {
          rodFile << std::setprecision(2) << fixed
                  << rods[j]->getPCMat((float)(i)) -
...
Simulator::Simulator(Settings* properties) {
  ...
  for (int i = 0; i < 8; i++) state_vector_[i] = new double[dataPoints];
...
void Simulator::init() {
  ...
  time_[0] = 0.;
  reactivity_[0] =
      getTotalRodReactivity() + core_excess_reactivity - getTotalRodWorth();
  rodReactivity_[0] = reactivity_[0];
  state_vector_[0][0] =
      -1e5 * getCurrentSourceActivity() * prompt_lifetime / rodReactivity_[0];
...
Simulator::~Simulator() {
  delete time_;
  delete reactivity_;
  for (int i = 0; i < 8; i++) delete state_vector_[i];
  delete xenon_;
  delete iodine_;
  delete temperature_;
...
const double& Simulator::getMaxFissionCrossSection() const { return Sigma_f; }

void Simulator::setMaxFissionCrossSection(const double& value) {
  Sigma_f = value;
}

const double& Simulator::getReactorCoreVolume() const { return core_volume; }
...
void Simulator::setNeutronSourceMode(const SimulationModes& value) {
  ...
  if (source_mode != SimulationModes::None)
    getSourceModeClass(source_mode)->reset();
}
...
double Simulator::getCurrentSourceActivity() {
  if (source_mode == SimulationModes::None) {
    ...
    return std::max(
        ns_base_activity + getSourceModeClass(source_mode)->getCurrentOffset(),
        0.);
  }
}
...
const double* Simulator::getNeutronGroup(size_t i) { return state_vector_[i]; }

double Simulator::getCurrentReactivity() const {
  return reactivity_[getCurrentIndex()];
}

const float* Simulator::getReactivity() const { return reactivity_; }

float Simulator::getCurrentRodReactivity() const {
  return rodReactivity_[getCurrentIndex()];
}

const float* Simulator::getRodReactivity() const { return rodReactivity_; }

float Simulator::getCurrentTemperature() const {
  return temperature_[getCurrentIndex()];
}

const float* Simulator::getTemperature() const { return temperature_; }

double* Simulator::getWaterTemperature() { return &waterTemperature; }

void Simulator::waterHeatingCycle(double dt) {
  double waterCapacity =
      waterVolume * 1e3 * water_specific_heat;  // C_vode = V * rho * c_v
  // Create derivatives
  double T_fuel = getCurrentPower() / waterCapacity;  // dT = P*dt/C_vode
  // From Žerovnik power calibration
  double Q_air =
      13.6 * pow(ENVIRONMENT_TEMPERATURE_DEFAULT - waterTemperature, 4 / 3);
  double Q_concrete =
      250 * (ENVIRONMENT_TEMPERATURE_DEFAULT - WaterTemperature);
  double T_passive_cooling = (Q_air + Q_concrete) / waterCapacity;
  // dT = (T-air_temp) * const. * dt
  // double T_passive_cooling = (waterTemperature -
  // ENVIRONMENT_TEMPERATURE_DEFAULT) * (cooling_coefficient / waterCapacity);
  double T_active_cooling =
      w_cooling ? (cooling_p / waterCapacity) : 0.;  // dT = P*dt / C_vode
  double wt_temp =
      waterTemperature + dt * (T_fuel - T_passive_cooling - T_active_cooling);
  if (wt_temp < ENVIRONMENT_TEMPERATURE_DEFAULT) {
    ...
    double delta_water_level = (Q / water_vaporization_ethalpy) /
                               (M_PI * std::pow(reactor_vessel_radius, 2.));
    if (!status)
      waterLevel_delta -=
          std::max(delta_water_level, -6.);  // Limit water level to -6 meters
  } else {
    waterTemperature = wt_temp;
  }
}

double* Simulator::getWaterLevel() { return &waterLevel_delta; }

void Simulator::recalculateDoseRate(double flux) {
  double theta =
      atan(TANK_RADIUS / WATER_HEIGHT);  // maximum angle an outward trajectory
                                         // can have with the vertical axis
  flux = flux * pow(theta / M_PI,
                    2);  // phi = phi0 * theta^2 ; probability that neutron is
                         // on outward trajectory multiplied with the gross flux
  double avgPath =
      WATER_HEIGHT *
      sqrt(tan(theta) / theta);  // average path length of neutron to surface
  flux = flux *
         exp(WATER_ABSORBTION *
             avgPath);  // exponential decrease of flux intensity through water
  doseRate = flux;
}
...
double Simulator::getFuelCp(double T) {
  ...
  // Volume of a signle fuel element  ((0.5 * 3.7338cm)**2 - (0.5 *
  // 0.635cm)**2)*3.14*38.1cm 3.556 cm is outer radius 0.635 cm is inner radius
  // 38.1  cm is fuel element len.
  // Geometry data from http://www.rcp.ijs.si/ric/description-s.html
  const double fuel_volume = (pow(0.5 * 3.556, 2) - pow(0.5 * 0.635, 2)) *
                             M_PI * 38.1 * (float)no_fuel_elements;
...
double Simulator::getStableTemperature(double P) {
  P /= no_fuel_elements;
  return waterTemperature + tempModelCoeff[0] * P +
             tempModelCoeff[1] * pow(P, 2),
         +tempModelCoeff[2] * pow(P, 3);
}

double Simulator::getCurrentTime() const { return time_[getCurrentIndex()]; }

const double* Simulator::getTime() const { return time_; }

const size_t& Simulator::getCalculationsPerformed() const {
  return calc_performed;
}

size_t Simulator::getOrderChanges() const { return powerExtremes->size(); }

Simulator::PowerExtreme& Simulator::getExtremeAt(size_t i) {
  return powerExtremes->at(i);
}

bool Simulator::isPaused() const { return speedFactor == 0.; }
...
void Simulator::setSevereErrorCallback(
    const std::function<void(int)>& callback) {
  severeErrorCallback = callback;
}

void Simulator::setPulseCallback(
    const std::function<void(PulseData)>& callback) {
  pulseCallback = callback;
}

void Simulator::setSpeedFactor(double value) { this->speedFactor = value; }

double& Simulator::getSpeedFactor() { return speedFactor; }
...
double& Simulator::getPowerLimit() { return powerLimit; }

void Simulator::setPowerLimit(double limit) { powerLimit = limit; }

double& Simulator::getPeriodLimit() { return periodLimit; }

void Simulator::setPeriodLimit(double limit) { periodLimit = limit; }

double& Simulator::getFuelTemperatureLimit() { return fuelTemperatureLimit; }

void Simulator::setFuelTemperatureLimit(double limit) {
  fuelTemperatureLimit = limit;
}

double& Simulator::getWaterTemperatureLimit() { return waterTemperatureLimit; }

void Simulator::setWaterTemperatureLimit(double limit) {
  waterTemperatureLimit = limit;
}

double& Simulator::getWaterLevelLimit() { return waterLevelLimit; }

void Simulator::setWaterLevelLimit(double limit) { waterLevelLimit = limit; }
    ...
    if (godMode) {
      ...
      if (periodTimer == -1)  // If period hasn't gone past the limit yet
      {
        ...
        if (reactorPeriod > periodLimit || reactorPeriod < 0) {
          ...
          if (periodTimer + 0.5 <
              getCurrentTime())  // If it's been there for more than half a
                                 // second, SCRAM!
          {
            if (period_scram_enabled &&
                !(regulatingRod()->getOperationMode() ==
                  ControlRod::OperationModes::Simulation))
              scram(ScramSignals::Period);
          }
        }
      }
    }
...
double* Simulator::getReactorPeriod() { return &reactorPeriod; }

double* Simulator::getReactorAsymPeriod() { return &reactorAsymPeriod; }

float Simulator::getTotalRodReactivity() {
  float sum = 0.f;
  for (int i = 0; i < NUMBER_OF_CONTROL_RODS; i++)
    sum += rods[i]->getCurrentPCM();
  return sum;
}
...
float Simulator::getTotalRodWorth() {
  float sum = 0.f;
  for (int i = 0; i < NUMBER_OF_CONTROL_RODS; i++)
    sum += rods[i]->getRodWorth();
  return sum;
}

void Simulator::runLoop() {
  double time = get_seconds_since_epoch();
  size_t srt_iterations;
  if (startTime < 0.) {
    startTime = get_seconds_since_epoch();
    srt_iterations = 1;
  } else {
    double processTime = (time - lastTime) *
                         speedFactor;  // the amount of time we need to process
    // itterations from the time we need to process plus the difference between
    // the actual simulator time and the latest time we simulated
    srt_iterations = (size_t)floor(
        (processTime + simulatorTime - time_[getCurrentIndex()]) / DT_STEP);
  ...
  frames_total++;
  lastTime = time;
  actualTime = time - startTime;  // Maybe we will use this some time in the
                                  // future, doesn't hurt fps so why not
}

const float rodAutoMove = 0.001f;  // how much can the control rod move at a
                                   // time (raw fraction of rodSteps)[0.1%]
void Simulator::mainLoop(size_t iterations) {
  // Optimizations:
  size_t currentIndex, nextIndex;
  double newPower, tempPow, negative_reactivity, rho, lastState[7], kf[4][7],
      finalState[8];
  ...
  for (size_t i = 0; i < iterations; i++) {
    ...
    for (int order = 0; order < 3; order++)
      stationary_temperature +=
          (float)(tempModelCoeff[order] * pow(tempPow, order + 1));
    new_temperature = temperature_[currentIndex];

    double power_losses = getCoolingFromTemperature(new_temperature);
    new_temperature +=
        (newPower - power_losses) * DT_STEP / getFuelCp(new_temperature);
    new_temperature = std::max(new_temperature, 22.);
    // The cooling step, performed in both FH model and asymptotic model,
    // commented out due to temperature model refractoring

    temperature_[nextIndex] = static_cast<float>(new_temperature);

    // Move rods
    // In the automatic mode, the rods are moved to reach or maintain a constant
    // power
    if (regulatingRod()->getOperationMode() ==
        ControlRod::OperationModes::Automatic) {
      ...
      if (std::abs(powerToKeep - newPower) / powerToKeep > steadyDeviation) {
        ...
        if ((avoidPeriodScram &&
             (reactorPeriod > periodLimit * 1.1 || reactorPeriod < 0.)) ||
            !avoidPeriodScram || (powerToKeep < newPower)) {
    ...
    rodReactivity_[nextIndex] =
        -getTotalRodWorth() + getTotalRodReactivity() + core_excess_reactivity;

    // The fission poison concentrations are changing slowly, so they do not
    // need to be calculated as often as the point kinetics. Default is each 125
    // steps
    if (nextIndex % 125 == 0) recalculatePoisonConcentrations(125 * DT_STEP);
    ...
    if (temperature_effects) {
      negative_reactivity +=
          getReactivityCoefficient(new_temperature) *
          (new_temperature - ENVIRONMENT_TEMPERATURE_DEFAULT);
    }
    if (fissionPoisoning_effects) {
      ...
      "; std::cout << Xe_conc * 1e5 * sigma_Xe_a / (nu_bar * Sigma_f) << " pcm
      conv "; std::cout <<	sigma_Xe_a / (nu_bar * Sigma_f) << " flux " <<
      getCurrentFlux() << std::endl;
      }
      */
    }

    // Substract total negative reactivity from insrted reactivity
    reactivity_[nextIndex] =
        rodReactivity_[nextIndex] - (float)(negative_reactivity);
    ...
    finalState[7] = 0.;
    for (int f = 0; f < 7; f++) finalState[7] += finalState[f];

    if ((finalState[0] - lastState[0]) *
            (lastState[0] - state_vector_[0][shiftIndex(currentIndex, -1)]) <
        0.)
...
void Simulator::recalculateLambdaBetaEffective() {
  ...
  for (int i = 0; i < 6; i++) {
    groupStability[i] =
        beta_neutrons[i] / (delayed_decay_time[i] * prompt_lifetime);
...
void Simulator::solvePerFrame() {
  ...
  int averageValues = (int)std::min(iterations_total - resetAverage - 1,
                                    (size_t)NO_AVERAGES_REACTIVITY);
  if (averageValues > 1) {
    const double periodSum =
        (1 - std::pow(periodK, averageValues - 2)) / (1 - periodK);
    ...
    for (int i = 0; i < averageValues - 1; i++) {
      sum += std::pow(periodK, averageValues - i - 2) /
             std::log(vals[i + 1] / vals[i]);
    }
    delete[] vals;
  ...
  if (this->delete_old_data_time > 0) {
    size_t deleteIndex = (dataPoints > iterations_total)
                             ? (size_t)0
                             : (iterations_total - dataPoints);
    if (deleteIndex > 0) {
...
void Simulator::neutronChange(double* new_state, double* prevState,
                              double rho) {
  fissionRate = prevState[0] / prompt_lifetime;
  temp_state[0] =
      (rho - beta_ /**1.15*/) * fissionRate;  // New fission neutrons per second

  if (source_inserted)
    temp_state[0] += ns_activity_temp;  // Neutron source neutrons per second

  for (int i = 0; i < 6; i++) {
    delayed = delayed_decay_time[i] *
              prevState[i + 1];  // Delayed neutrons per second
    if (delayed_enabled[i]) temp_state[0] += delayed;
    temp_state[i + 1] = (beta_neutrons[i] * fissionRate) - delayed;
  }

  for (int i = 0; i < 7; i++) new_state[i] = temp_state[i];
}
...
void Simulator::pushStableState(double power) {
  ...
  double waterCapacity =
      waterVolume * 1e3 * water_specific_heat;  // C_vode = V * rho * c_v
  float stableWaterTemp =
      (float)(power / (cooling_coefficient / waterCapacity)) +
      WATER_TEMPERATURE_DEFAULT;
  // float stableWaterTemp = (float)(power / (waterVolume * 41.855 * 1.3)) +
  // WATER_TEMPERATURE_DEFAULT;
  ...
  float stableFuelTemp =
      (float)(stableWaterTemp + tempModelCoeff[0] * me +
  ...
  if (source_inserted)
    reactivityToInsert -=
        (float)(1e5 * ns_base_activity * prompt_lifetime * t_neutron_speed *
                Sigma_f / (power * fissions_per_wattSecond));
  if (temperature_effects)
    reactivityToInsert +=
        getReactivityCoefficient(stableFuelTemp) *
        (stableFuelTemp - (float)ENVIRONMENT_TEMPERATURE_DEFAULT);
  float rodPos[NUMBER_OF_CONTROL_RODS] = {
      safetyRod()->getPosFromPcm(reactivityToInsert), 0.f, 0.f};
  ...
  cout << "Set stable mode at " << power << "W" << endl
       << "Reactivity overkill: " << reactivityToInsert << "pcm" << endl;
...
void Simulator::setProperties(Settings* nodes) {
  ...
  for (size_t rodIndex = 0; rodIndex < NUMBER_OF_CONTROL_RODS; rodIndex++) {
    ...
    for (size_t i = 0; i < 2; i++) {
      rods[rodIndex]->setParameter(i, nodes->rodSettings[rodIndex].rodCurve[i],
                                   false);
    }
  ...
  for (int sqw = 0; sqw < 4; sqw++)
    rods[1]->squareWave()->xIndex[sqw] = nodes->squareWave.xIndex[sqw];
  if (nodes->squareWaveUsesRodSpeed)
    rods[1]->squareWave()->rodSpeed = rods[1]->getRodSpeed();
  ...
  for (int saw = 0; saw < 6; saw++)
    rods[1]->sawTooth()->xIndex[saw] = nodes->sawToothMode.xIndex[saw];
  ...
  for (int sqw = 0; sqw < 4; sqw++)
    source_sqw->xIndex[sqw] = nodes->ns_squareWave.xIndex[sqw];
  ...
  for (int saw = 0; saw < 6; saw++)
    source_saw->xIndex[saw] = nodes->ns_sawToothMode.xIndex[saw];
...
void Simulator::resetSimulator() { reset(nullptr); }
    ...
    for (auto i = scriptCommands.begin(); i != scriptCommands.end();) {
      if (i->timed <= getCurrentTime()) {
        switch (i->command) {
          ...
          cout << "Running action number" << i->command << " name "
               << i->strCommand << endl;
          ...
          case setStablePower:
            ...
            regulatingRod()->setOperationMode(
          ...
          case setSimulationMode:
            ...
            if (i->value == "Pulse")
              regulatingRod()->setOperationMode(
                  ControlRod::OperationModes::Pulse);
...
void Simulator::addPowerExtremes() {
  try {
    ...
    if (last_sample_number != 0) {
      ...
      if (dataStart == 1) {
        powerExtremes->push_back(
            PowerExtreme((int)std::floor(std::log10(getCurrentPower())),
                         time_[getOldestIndex()]));
        dataStart++;
      }
      ...
      for (int i = (int)last_sample_number - 1; i >= 0; i--) {
        power_i = powerFromNeutrons(
            state_vector_[0][shiftIndex(getCurrentIndex(), -i)]);
        if (power_i == 0.) {
          bool setZero = false;
          if (powerExtremes->size()) {
          ...
          int lastOrder = powerExtremes->size() ? powerExtremes->back().order
                                                : trailingExtreme.order;

          // If the order changed, save the index and new order
          if (order != lastOrder) {
            powerExtremes->push_back(
                PowerExtreme(order, time_[shiftIndex(getCurrentIndex(), -i)]));
          }
        }
      }
    }
...
void Simulator::checkPulsingStatus() {
  ...
  if (pulsing) {
    const size_t currentIdx = getCurrentIndex();
    if (time_[currentIdx] - time_[pulse_start] >=
        5.) {  // check if pulse is finished
      pulsing = false;
      ...
      for (int i = 0; i < 5000; i++) {
        if (range[0] < 0) {
          if (state_vector_[0][shiftIndex(currentIdx, i - 4999)] >
              (pulse_maxP + currentPower) / 2.)
            range[0] = i - 1;
        } else {
          if (state_vector_[0][shiftIndex(currentIdx, i - 4999)] <
              (pulse_maxP + currentPower) / 2.) {
      ...
      // tempMode = TemperatureMode::Asymptotic; // Revert from FH to the
      // original model
