...
void SimulatorGUI::updatePulseTrack(bool updateData) {
  ...
  startIdx = reactor->getIndexFromTime(
      reactor->time_[lastPulseData.pulseStartIndex] + pulseTimer->value(0) * 5);
  endIdx = reactor->getIndexFromTime(
      reactor->time_[lastPulseData.pulseStartIndex] + pulseTimer->value(1) * 5);
  ...
  for (int i = 0; i < 4; i++) {
    ...
    if (i == 3) {
      pair<int, int> orders =
          recalculatePowerExtremes(timeLimits[0], timeLimits[1]);
      pulsePlots[i]->setLimits(timeLimits[0], timeLimits[1], 0.,
                               std::pow(10., orders.second));
      bool maxVisible = (lastPulseData.timeAtMax >= timeLimits[0]) ||
                        (lastPulseData.timeAtMax <= timeLimits[1]);
      if (maxVisible) {
        pulsePlots[i]->setHorizontalPointerPosition(
            (float)((lastPulseData.timeAtMax - timeLimits[0]) /
                    (timeLimits[1] - timeLimits[0])));
        pulsePlots[i]->setPointerPosition(
            (float)(lastPulseData.peakPower / pulsePlots[i]->limits()[3]));
      }
      pulsePlots[i]->setHorizontalPointerShown(maxVisible);
      pulsePlots[i]->setPointerShown(maxVisible);
      pulsePlots[i]->setLimitOverride(
          0, pulseTimer->value(0)
                 ? (to_string((int)(pulseTimer->value(0) * 5e3)) + "ms")
                 : "pulse start");
      pulsePlots[i]->setLimitOverride(
          1, to_string((int)(pulseTimer->value(1) * 5e3)) + "ms");
    } else if (!(i % 2)) {
      pulsePlots[i]->setLimits(
          timeLimits[0], timeLimits[1],
          std::floor(reactor->reactivity_[startIdx] / 200.) * 200,
          std::ceil(reactor->rodReactivity_[endIdx] / 200.) * 200);
    } else {
      pulsePlots[i]->setLimits(
          timeLimits[0], timeLimits[1], 0.f,
          updateData ? (std::ceil(lastPulseData.maxFuelTemp / 200.) * 200)
                     : pulsePlots[i]->limits()[3]);
    }
  }

  const int per[2] = {(int)roundf(fmodf(pulseTimer->value(0) * 5, 1.f) * 100),
                      (int)roundf(fmodf(pulseTimer->value(1) * 5, 1.f) * 100)};
  pulseDisplayLabels[0]->setCaption(
      std::to_string((int)(pulseTimer->value(0) * 5)));
  pulseDisplayLabels[1]->setCaption(
      (per[0] < 10) ? "0" : "" + std::to_string(per[0]));
  pulseDisplayLabels[2]->setCaption(
      std::to_string((int)(pulseTimer->value(1) * 5)));
  pulseDisplayLabels[3]->setCaption(
      (per[1] < 10) ? "0" : "" + std::to_string(per[1]));

  if (updateData) {
    pulseLabels[0]->setCaption(
        formatDecimalsDouble(lastPulseData.peakPower * 1e-6, 1) + " MW");
    pulseLabels[1]->setCaption(to_string((int)(lastPulseData.FWHM * 1e3)) +
                               " ms");
    pulseLabels[2]->setCaption(
        formatDecimalsDouble(lastPulseData.maxFuelTemp, 1) + " " +
        degCelsiusUnit);
    pulseLabels[3]->setCaption(
        formatDecimalsDouble(lastPulseData.releasedEnergy * 1e-6, 1) + " MJ");
  }
}

void SimulatorGUI::createDataDisplays(Widget *parent,
                                      RelativeGridLayout *rLayout) {
  // Create the displays
  displayPanel1 = parent->add<CustomWidget>();
  displayPanel1->setLayout(
      new BoxLayout(Orientation::Vertical, Alignment::Fill, 10, 5));
  rLayout->setAnchor(displayPanel1,
  ...
  displayPanel2->setLayout(
      new BoxLayout(Orientation::Vertical, Alignment::Fill, 10, 5));
  rLayout->setAnchor(displayPanel2,
                     RelativeGridLayout::makeAnchor(
                         1, 0, 1, 1, Alignment::Fill, Alignment::Fill,
                         RelativeGridLayout::FillMode::Always,
                         RelativeGridLayout::FillMode::IfLess));
  ...
  powerShow->setUnit("W");
  reactivityShow =
      displayPanel2->add<DataDisplay<float>>("Reactivity (actual):");
  ...
  rodReactivityShow =
      displayPanel2->add<DataDisplay<float>>("Reactivity (inserted):");
  ...
  waterTemperatureShow =
      displayPanel2->add<DataDisplay<double>>("Water temperature:");
  ...
  waterLevelShow =
      displayPanel1->add<DataDisplay<double>>("Water level change:");
...
void SimulatorGUI::initializeGraph() {
  ...
  canvas->setPadding(90.f, 25.f, properties->reactivityHardcore ? 120.f : 220.f,
                     50.f);
  ...
  //	canvas->getPlot(i)->setPlotRange(displayInterval[0],
  //displayInterval[1]);
  //}
}
...
void SimulatorGUI::initializePulseGraph() {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    pulsePlots[i]->setNumberFormatMode(
        (i < 3) ? GraphElement::FormattingMode::Normal
                : GraphElement::FormattingMode::Exponential);
