...
void SimulatorGUI::loadScriptFromFile(std::string path) {
  ...
  if (path.length()) {
    ...
    msg->setCallback([this](int /*choice*/) { toggleBaseWindow(true); });
  } else {
    MessageDialog *msg = new MessageDialog(this, MessageDialog::Type::Warning,
                                           "Load script", "Bad file name.");
    msg->setPosition(Vector2i((this->size().x() - msg->size().x()) / 2,
                              (this->size().y() - msg->size().y()) / 2));
    msg->setCallback([this](int /*choice*/) { toggleBaseWindow(true); });
  }
...
void SimulatorGUI::updateSettings(bool updateReactor) {
  ...
  coreVolumeBox->setValue(properties->coreVolume *
                          1e3);  // Convert from m3 to L
  ...
  neutronSourceAmplitudeBoxes[2]->setValue(
      properties->ns_sawToothMode.amplitude);
  for (int i = 0; i < 4; i++)
    neutronSourceSQWBoxes[i]->setValue(
        (int)(properties->ns_squareWave.xIndex[i] * 100));
  neutronSourceSINEModeBox->setSelectedIndex((int)properties->ns_sineMode.mode);
  for (int i = 0; i < 6; i++)
    neutronSourceSAWBoxes[i]->setValue(
        (int)(properties->ns_sawToothMode.xIndex[i] * 100));
  ...
  for (int i = 0; i < 3; i++) {
    ...
    for (int j = 0; j < 2; j++) {
      rodCurveSliders[i * 2 + j]->setValue(
          properties->rodSettings[i].rodCurve[j]);
      rodCurveSliders[i * 2 + j]->finalCallback()(
          properties->rodSettings[i].rodCurve[j]);
