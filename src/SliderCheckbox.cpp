	...
	if (button == GLFW_MOUSE_BUTTON_1)
	{
		...
		else if (mPushed)
		{
			if (contains(p))
			{
				...
				{
					state = -state;
					timeAtChange = 2 * get_seconds_since_epoch() - timeAtChange - ANIM_TIME;
				}
				if (mCallback)
					mCallback(mChecked);
			}
...
{
	// Note: SliderCheckBox has no children, skip Widget::draw() to avoid scissor clipping
...
bool SliderCheckBox::load(Serializer &s)
{
	if (!Widget::load(s))
		return false;
	if (!s.get("pushed", mPushed))
		return false;
	if (!s.get("checked", mChecked))
		return false;
	return true;
}
