...
Vector2i RelativeGridLayout::preferredSize(NVGcontext* ctx,
  ...
  for (Widget* w : widget->children()) {
    ...
    for (size_t axis = 0; axis < 2; axis++) {
      if (mTable[axis][a.pos[axis]].type() == SizeType::Relative) {
        tempTable[axis][a.pos[axis]] =
            std::max(tempTable[axis][a.pos[axis]], ps[axis]);
      } else {
        continue;
      }
    }
  ...
  if (dynamic_cast<const Window*>(widget))
    yOffset += widget->theme()->mWindowHeaderHeight - mMargin / 2.f;
  return Vector2i(fixedSum[0] + relativeFill[0],
                  fixedSum[1] + relativeFill[1] + (int)std::roundf(yOffset));
}
...
void RelativeGridLayout::performLayout(NVGcontext* ctx, Widget* widget) const {
  for (Widget* w : widget->children()) {
    ...
    Vector4i bounds = boundsOf(widget, anchor.pos[0], anchor.pos[1],
                               anchor.span[0], anchor.span[1]);
    ...
    for (size_t axis = 0; axis < 2; axis++) {
      ...
      switch (anchor.alignment[axis]) {
        ...
        case Alignment::Middle:
          usePos[axis] = bounds[axis] + (bounds[axis + 2] - useSize[axis]) / 2 +
                         anchor.padding[axis];
          break;
        case Alignment::Maximum:
          usePos[axis] = bounds[axis] + (bounds[axis + 2] - useSize[axis]) -
                         anchor.padding[2 + axis];
          break;
        case Alignment::Fill:
          usePos[axis] = bounds[axis] + anchor.padding[axis];
          switch (anchor.expansion[axis]) {
            case FillMode::Always:
              useSize[axis] = bounds[axis + 2] - anchor.padding[axis] -
                              anchor.padding[2 + axis];
              break;
            case FillMode::IfLess:
              if (ps[axis] < bounds[axis + 2] - anchor.padding[axis] -
                                 anchor.padding[2 + axis]) {
                useSize[axis] = bounds[axis + 2] - anchor.padding[axis] -
                                anchor.padding[2 + axis];
              }
...
RelativeGridLayout::Anchor RelativeGridLayout::makeAnchor(
    int x, int y, int w, int h, Alignment alignX, Alignment alignY,
    FillMode modeX, FillMode modeY) {
...
Vector2f RelativeGridLayout::positionOf(const Widget* widget, size_t x,
                                        size_t y) const {
  ...
  for (size_t axis = 0; axis < 2; axis++) {
    ...
    if (relativeSum[axis] != 0.f)
      accumulateRelative[axis] *=
          (float)(widget->size()[axis] - fixedSum[axis]) / relativeSum[axis];
  }

  return accumulateFixed + accumulateRelative;
}

Vector4i RelativeGridLayout::boundsOf(const Widget* widget, size_t x, size_t y,
                                      size_t w, size_t h) const {
  Vector2f source = positionOf(widget, x, y);
  Vector2f end = positionOf(widget, x + w, y + h);
  return Vector4i((int)std::roundf(source.x()), (int)std::roundf(source.y()),
                  (int)std::roundf(end.x() - source.x()),
                  (int)std::roundf(end.y() - source.y()));
}
