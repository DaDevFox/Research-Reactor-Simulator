...
void SimulatorGUI::tryConnectingTo(string port) {
  ...
  try {
    ...
    std::cout << "Exception while astablishing connection with " << port << "!"
              << std::endl;
  }
  bool flag = false;
...
void SimulatorGUI::handleBox() {
  ...
  if (reactor->safetyRod()->getCommandType() == ControlRod::CommandType::Top ||
      *reactor->safetyRod()->getExactPosition() ==
          (float)*reactor->safetyRod()->getRodSteps()) {
    LEDstatus |= ROD_SAFETY_UP;
  }
  if (reactor->regulatingRod()->getCommandType() ==
          ControlRod::CommandType::Top ||
      *reactor->regulatingRod()->getExactPosition() ==
          (float)*reactor->regulatingRod()->getRodSteps()) {
    LEDstatus |= ROD_REG_UP;
  }
  if (reactor->shimRod()->getCommandType() == ControlRod::CommandType::Top ||
      *reactor->shimRod()->getExactPosition() ==
          (float)*reactor->shimRod()->getRodSteps()) {
    LEDstatus |= ROD_SHIM_UP;
  }
  if (reactor->safetyRod()->getCommandType() ==
          ControlRod::CommandType::Bottom ||
      *reactor->safetyRod()->getExactPosition() == 0.f) {
    LEDstatus |= ROD_SAFETY_DOWN;
  }
  if (reactor->regulatingRod()->getCommandType() ==
          ControlRod::CommandType::Bottom ||
      *reactor->regulatingRod()->getExactPosition() == 0.f) {
    LEDstatus |= ROD_REG_DOWN;
  }
  if (reactor->shimRod()->getCommandType() == ControlRod::CommandType::Bottom ||
      *reactor->shimRod()->getExactPosition() == 0.f) {
    LEDstatus |= ROD_SHIM_DOWN;
  }
  if (reactor->regulatingRod()->getOperationMode() ==
          ControlRod::OperationModes::Pulse &&
      reactor->getScramStatus() == 0) {
...
void SimulatorGUI::handleBoxData(uint16_t box_data, double now) {
  ...
  for (int i = 0; i < NUMBER_OF_CONTROL_RODS; i++)
    rodsMoving[i] =
        (reactor->rods[i]->getCommandType() == ControlRod::CommandType::None);
  ...
  if (box_data & UP_SAFETY_BTN) {
    if (!btns[3] &&
        ((rodsMoving[1] && rodsMoving[2]) || properties->allRodsAtOnce))
  ...
  if (box_data & UP_REG_BTN) {
    if (!btns[6] &&
        ((rodsMoving[0] && rodsMoving[2]) || properties->allRodsAtOnce))
  ...
  if (box_data & UP_SHIM_BTN) {
    if (!btns[9] &&
        ((rodsMoving[0] && rodsMoving[1]) || properties->allRodsAtOnce))
