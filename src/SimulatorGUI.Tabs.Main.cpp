...
void SimulatorGUI::createMainTab() {
  ...
  rel3->appendCol(RelativeGridLayout::Size(
      250.f, RelativeGridLayout::SizeType::Fixed));  // 0 first column
  rel3->appendCol(RelativeGridLayout::Size(
      250.f, RelativeGridLayout::SizeType::Fixed));  // 1 second column
  rel3->appendCol(RelativeGridLayout::Size(
      80.f, RelativeGridLayout::SizeType::Fixed));  // 2 reactivitycolumn
  rel3->appendCol(RelativeGridLayout::Size(
      7.f, RelativeGridLayout::SizeType::Fixed));  // 3 padding
  rel3->appendCol(RelativeGridLayout::Size(
      190.f, RelativeGridLayout::SizeType::Fixed));  // 4 control rod display
  rel3->appendCol(RelativeGridLayout::Size(
      7.f, RelativeGridLayout::SizeType::Fixed));  // 5 padding
  rel3->appendCol(1.f);                            // 6 main content
  rel3->appendCol(RelativeGridLayout::Size(
      100.f, RelativeGridLayout::SizeType::Fixed));  // 7 alarms
  rel3->appendRow(1.f);
  ...
  main_right->setLayout(
      new BoxLayout(Orientation::Vertical, Alignment::Fill, 10, 10));

  // Create a panel for reactivity controls
  /*Widget* reactivityPanel = main_right->add<Widget>();
  reactivityPanel->setLayout(new BoxLayout(Orientation::Horizontal,
  ...
  unscram->setCallback(
      [this] { reactor->scram(Simulator::ScramSignals::None); });
  ...
  alarmLayout->appendRow(
      RelativeGridLayout::Size(35.f, RelativeGridLayout::SizeType::Fixed));
  alarmPanel->setLayout(alarmLayout);

  CustomLabel *alarmHeader = alarmPanel->add<CustomLabel>("SCRAMs");
  alarmHeader->setFontSize(24.f);
  alarmHeader->setTextAlignment(CustomLabel::TextAlign::HORIZONTAL_CENTER |
                                CustomLabel::TextAlign::VERTICAL_CENTER);
  ...
  for (int i = 0; i < 6; i++) {
    ...
    alarmLabels[i]->setTextAlignment(CustomLabel::TextAlign::HORIZONTAL_CENTER |
                                     CustomLabel::TextAlign::VERTICAL_CENTER);
  ...
  // Controls
  {
    for (int i = 0; i < NUMBER_OF_CONTROL_RODS; i++) {
      rodBox[i] = makeSettingLabel<IntBox<int>>(
          main_right,
          reactor->rods[i]->getRodName() + " rod magnet position: ", 200);
  ...
  // Checkboxes
  {
    ...
    checkBoxPanelNeutronSource->add<CustomLabel>("Neutron source :",
                                                 "sans-bold");
...
void SimulatorGUI::createDelayedGroupsTab() {
  ...
  topLayout->appendCol(RelativeGridLayout::Size(
      158.f, RelativeGridLayout::SizeType::Fixed));  // key
  topLayout->appendRow(1.f);
  topHost->setLayout(topLayout);

  Color dataColors[7] = {Color(255, 255),         Color(34, 116, 165, 255),
                         Color(247, 92, 3, 255),  Color(241, 196, 15, 255),
                         Color(0, 204, 102, 255), Color(240, 58, 71, 255),
                         Color(153, 0, 153, 255)};

  CustomWidget *keyPanel = topHost->add<CustomWidget>();
  topLayout->setAnchor(
      keyPanel, RelativeGridLayout::makeAnchor(1, 0, 1, 1, Alignment::Minimum,
                                               Alignment::Middle));
  keyPanel->setDrawBackground(true);
  keyPanel->setBackgroundColor(Color(65, 255));
  keyPanel->setBorder(Border::BOTTOM | Border::LEFT | Border::TOP |
                      Border::RIGHT);
  ...
  rel->appendCol(RelativeGridLayout::Size(
      50.f, RelativeGridLayout::SizeType::Fixed));  // colors
  rel->appendCol(RelativeGridLayout::Size(
      1.f, RelativeGridLayout::SizeType::Fixed));  // border
  rel->appendCol(RelativeGridLayout::Size(
      10.f, RelativeGridLayout::SizeType::Fixed));  // local padding
  rel->appendCol(RelativeGridLayout::Size(
      70.f, RelativeGridLayout::SizeType::Fixed));  // text
  for (int i = 0; i < 7; i++)
    rel->appendRow(RelativeGridLayout::Size(
        35.f, RelativeGridLayout::SizeType::Fixed));  // data rows
  rel->appendRow(RelativeGridLayout::Size(
      5.f, RelativeGridLayout::SizeType::Fixed));  // padding
  keyPanel->setLayout(rel);
  Label *tempLabelKey = keyPanel->add<Label>("Color", "sans-bold");
  rel->setAnchor(tempLabelKey,
                 RelativeGridLayout::makeAnchor(0, 0, 1, 1, Alignment::Middle,
                                                Alignment::Middle));
  tempLabelKey = keyPanel->add<Label>("Group No.", "sans-bold");
  rel->setAnchor(tempLabelKey,
                 RelativeGridLayout::makeAnchor(3, 0, 1, 1, Alignment::Minimum,
                                                Alignment::Middle));
  ...
  for (int i = 1; i < 7; i++) {
    ...
    rel->setAnchor(temp, RelativeGridLayout::makeAnchor(
                             0, i, 1, 1, Alignment::Middle, Alignment::Middle));
    temp->setDrawBackground(true);
    ...
    rel->setAnchor(tempLabel,
                   RelativeGridLayout::makeAnchor(
                       3, i, 1, 1, Alignment::Minimum, Alignment::Middle));
  }
  ...
  topLayout->setAnchor(delayedGroupsGraph,
                       RelativeGridLayout::makeAnchor(0, 0));
  // double betaLambda = reactor->prompt_lifetime;
  double betaLambda = 0;
  double sumBeta = 0, sumLambdaC = 0;
  /*
  The delayed group fractions for display are normalized to 100%, where 100% is
  a concentration of each group during stationary conditions at current power.
  */
  for (int i = 0; i < 6; i++) {
  ...
  for (int i = 0; i < 6; i++) {
    delayedGroups[i] =
        delayedGroupsGraph->addPlot(reactor->getDataLength(), true);
    ...
    delayedGroups[i]->setAxisPosition((i < 3)
                                          ? GraphElement::AxisLocation::Right
                                          : GraphElement::AxisLocation::Left);
    delayedGroups[i]->setPixelDrawRatio(
        0.3f);  // skip 70% of pixels when drawing
    delayedGroups[i]->setValueComputing(
        [this, sumBeta, sumLambdaC, i](double *val, const size_t index) {
          //*val = *val * sumLambdaC * reactor->delayed_decay_time[i] /
          //(reactor->state_vector_[7][index] * reactor->beta_neutrons[i]);
          double sumLambdaC = 0;
          for (int j = 0; j < 6; j++)
            sumLambdaC += reactor->delayed_decay_time[j] *
                          reactor->state_vector_[j + 1][index];
          *val = *val * sumBeta * reactor->delayed_decay_time[i] /
                 (sumLambdaC * reactor->beta_neutrons[i]);
        });
    if (!i) {
...
void SimulatorGUI::updateNeutronSourceTab() {
  ...
  PeriodicalMode *ns_mode =
      reactor->getSourceModeClass(reactor->getNeutronSourceMode());
  size_t dataP = ns_mode->num_points();
  neutronSourcePlot = sourceGraph->addPlot(dataP);
  neutronSourcePlot->setPlotRange(0, dataP - 1);
  neutronSourcePlot->setLimits(0., ns_mode->getPeriod(),
                               std::min(-1.5 * ns_mode->getAmplitude(), -1.),
                               std::max(1.5 * ns_mode->getAmplitude(), 1.));
