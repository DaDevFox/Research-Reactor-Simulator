...
void SimulatorGUI::createRodSettingsTab() {
  ...
  RelativeGridLayout *rod_settings_layout =
      new RelativeGridLayout(); /* COLUMNS */
  for (int i = 0; i < 9; i++) {
    if (i == 3 || i == 6)
      rod_settings_layout->appendCol(
          RelativeGridLayout::Size(2.f, RelativeGridLayout::SizeType::Fixed));
    rod_settings_layout->appendCol(1.f);
  }
  /* ROWS */
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      35.f, RelativeGridLayout::SizeType::Fixed));  // 0 title
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      15.f, RelativeGridLayout::SizeType::Fixed));  // 1 empty space
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      20.f, RelativeGridLayout::SizeType::Fixed));  // 2 setting name
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      20.f, RelativeGridLayout::SizeType::Fixed));  // 3 setting value
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      30.f, RelativeGridLayout::SizeType::Fixed));  // 4 top slider
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      320.f, RelativeGridLayout::SizeType::Fixed));  // 5 graph
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      30.f, RelativeGridLayout::SizeType::Fixed));  // 6 bottom slider
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      15.f, RelativeGridLayout::SizeType::Fixed));  // 7 empty space
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      1.f, RelativeGridLayout::SizeType::Fixed));  // 8 border
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      15.f, RelativeGridLayout::SizeType::Fixed));  // 9 empty space
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      20.f, RelativeGridLayout::SizeType::Fixed));  // 10 label
  rod_settings_layout->appendRow(RelativeGridLayout::Size(
      320.f, RelativeGridLayout::SizeType::Fixed));  // 11 derivative graph
  rod_settings->setLayout(rod_settings_layout);

  // Create borders
  for (int i = 0; i < 2; i++) {
    CustomWidget *border = rod_settings->add<CustomWidget>();
    rod_settings_layout->setAnchor(
        border, RelativeGridLayout::makeAnchor(i ? 3 : 7, 0, 1, 13));
    border->setBackgroundColor(coolBlue);
    border->setDrawBackground(true);
  }
  CustomWidget *border = rod_settings->add<CustomWidget>();
  rod_settings_layout->setAnchor(border,
                                 RelativeGridLayout::makeAnchor(0, 8, 11, 1));
  border->setBackgroundColor(coolBlue);
  border->setDrawBackground(true);
  ...
  for (int i = 0; i < NUMBER_OF_CONTROL_RODS; i++) {
    ...
    // Titles
    tempLabel =
        rod_settings->add<Label>(useRod->getRodName() + " rod", "sans-bold");
    tempLabel->setFontSize(30);
    rod_settings_layout->setAnchor(
        tempLabel, RelativeGridLayout::makeAnchor(
                       i * 4, 0, 3, 1, Alignment::Middle, Alignment::Maximum));

    // Rod steps setting
    tempLabel = rod_settings->add<Label>("Rod steps:", "sans-bold");
    rod_settings_layout->setAnchor(
        tempLabel, RelativeGridLayout::makeAnchor(
                       i * 4, 2, 1, 1, Alignment::Middle, Alignment::Middle));
    rodStepsBox[i] = rod_settings->add<IntBox<int>>(
        (int)properties->rodSettings[i].rodSteps);
    rod_settings_layout->setAnchor(
        rodStepsBox[i],
        RelativeGridLayout::makeAnchor(i * 4, 3, 1, 1, Alignment::Middle));
    rodStepsBox[i]->setFixedSize(Vector2i(100, 20));
    rodStepsBox[i]->setAlignment(TextBox::Alignment::Left);
    rodStepsBox[i]->setUnits("steps");
    rodStepsBox[i]->setDefaultValue(
        to_string(properties->rodSettings[i].rodSteps));
    ...
    rodStepsBox[i]->setCallback([useRod, i, this](int change) {
      ...
      rodCurves[i]->setLimitHorizontalMultiplier(
          (double)*useRod->getRodSteps());
    });
    ...
    rod_settings_layout->setAnchor(
        tempLabel,
        RelativeGridLayout::makeAnchor(i * 4 + 1, 2, 1, 1, Alignment::Middle,
                                       Alignment::Middle));
    rodWorthBox[i] =
        rod_settings->add<FloatBox<float>>(properties->rodSettings[i].rodWorth);
    rod_settings_layout->setAnchor(
        rodWorthBox[i],
        RelativeGridLayout::makeAnchor(i * 4 + 1, 3, 1, 1, Alignment::Middle));
    rodWorthBox[i]->setFixedSize(Vector2i(100, 20));
    rodWorthBox[i]->setAlignment(TextBox::Alignment::Left);
    rodWorthBox[i]->setUnits("pcm");
    rodWorthBox[i]->setDefaultValue(
        to_string(properties->rodSettings[i].rodWorth));
    ...
    rod_settings_layout->setAnchor(
        tempLabel,
        RelativeGridLayout::makeAnchor(i * 4 + 2, 2, 1, 1, Alignment::Middle,
                                       Alignment::Middle));
    rodSpeedBox[i] =
        rod_settings->add<FloatBox<float>>(properties->rodSettings[i].rodSpeed);
    rod_settings_layout->setAnchor(
        rodSpeedBox[i],
        RelativeGridLayout::makeAnchor(i * 4 + 2, 3, 1, 1, Alignment::Middle));
    ...
    rodSpeedBox[i]->setDefaultValue(
        to_string(properties->rodSettings[i].rodSpeed));
    ...
    rodSpeedBox[i]->setCallback([useRod, i, this](float change) {
      ...
      if (i == 1)
        reactor->regulatingRod()->sine()->fillXYaxis(
            operationModesPlots[0][1],
            operationModesPlots[1][1]);  // Update SQW graph
    });
    ...
    rod_settings_layout->setAnchor(
        rodControl,
        RelativeGridLayout::makeAnchor(i * 4, 5, 3, 1, Alignment::Middle));
    ...
    for (int j = 0; j < 2; j++) {
      ...
      rod_settings_layout->setAnchor(
          rodCurveSliders[sliderIndex],
          RelativeGridLayout::makeAnchor(i * 4, 4 + j * 2, 3, 1,
                                         Alignment::Middle, Alignment::Middle));
      rodCurveSliders[sliderIndex]->setFixedSize(Vector2i(340, 15));
      rodCurveSliders[sliderIndex]->setValue(
          properties->rodSettings[i].rodCurve[j]);
      rodCurveSliders[sliderIndex]->setCallback([this, i, j](float change) {
        rodCurves[i]->setParameter(j * 2, change);
      });
      rodCurveSliders[sliderIndex]->setFinalCallback(
          [useRod, j, i, this](float stop) {
    ...
    rod_settings_layout->setAnchor(
        tempLabel, RelativeGridLayout::makeAnchor(
                       i * 4, 10, 3, 1, Alignment::Middle, Alignment::Maximum));

    CustomGraph *rodDerDisplay = rod_settings->add<CustomGraph>(1);
    rod_settings_layout->setAnchor(
        rodDerDisplay,
        RelativeGridLayout::makeAnchor(i * 4, 11, 3, 1, Alignment::Middle,
                                       Alignment::Minimum));
    ...
    rodDerivatives[i]->setLimitHorizontalMultiplier(
        (double)*useRod->getRodSteps());
