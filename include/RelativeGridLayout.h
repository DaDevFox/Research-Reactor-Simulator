...
public:
	...
	struct Anchor
	{
		...
		nanogui::Alignment alignment[2] = { nanogui::Alignment::Fill, nanogui::Alignment::Fill };

		Anchor()
		{
			padding = nanogui::Vector4i(0, 0, 0, 0);
		}
		...
		Anchor(int x, int y, int w, int h, nanogui::Vector4i padding_) : Anchor(x, y, w, h)
		{
			padding = padding_;
		}
	...
	void setAnchor(const nanogui::Widget* widget, const Anchor& anchor) { mAnchor[widget] = anchor; }
	...
	virtual nanogui::Vector2i preferredSize(NVGcontext* ctx, const nanogui::Widget* widget) const override;
	virtual void performLayout(NVGcontext* ctx, nanogui::Widget* widget) const override;

	static Anchor makeAnchor(int x, int y, int w = 1, int h = 1, nanogui::Alignment alignX = nanogui::Alignment::Fill,
		nanogui::Alignment alignY = nanogui::Alignment::Fill, FillMode modeX = FillMode::Always, FillMode modeY = FillMode::Always);

protected:
	nanogui::Vector2f positionOf(const nanogui::Widget* widget, size_t x, size_t y) const;
	nanogui::Vector4i boundsOf(const nanogui::Widget* widget, size_t x, size_t y, size_t w, size_t h) const;
