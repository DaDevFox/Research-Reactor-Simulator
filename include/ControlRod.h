...
class ControlRod {
 public:
  enum OperationModes : std::uint8_t { Manual, Simulation, Automatic, Pulse };

  enum CommandType : std::uint8_t { None, Top, Bottom, Fixed };
 ...
 public:
  ...
  void recalculateStepData() {
    ...
    for (size_t i = 0; i <= INTEGRAL_CURVE_POINTS; i++) {
      ...
      xPoints[i] =
          t * (3 * (1 - t) * ((1 - t) * parameters[0] + t * parameters[1]) +
               pow(t, 2));
    ...
    for (size_t i = 1; i < rod_steps; i++) {
      ...
      stepData[i] = (float)(yPoints[currentIndex - 1] * (1 - alpha) +
                            yPoints[currentIndex] * alpha);
  ...
  ControlRod(bool recalculateSteps = true) {
    ...
    sqw = new SquareWave(SIMULATION_MODE_PERIOD_DEFAULT,
                         SIMULATION_MODE_AMPLITUDE_DEFAULT);
    sinMode = new Sine(SIMULATION_MODE_PERIOD_DEFAULT,
                       SIMULATION_MODE_AMPLITUDE_DEFAULT);
    saw = new SawTooth(SIMULATION_MODE_PERIOD_DEFAULT,
                       SIMULATION_MODE_AMPLITUDE_DEFAULT);
  }
  ...
  ~ControlRod() {
    ...
    delete sinMode;
    delete saw;
    if (hasData) delete[] stepData, derivativeTable;
  }
  ...
  void setRodSteps(size_t steps, bool recalculateSteps = true) {
    rod_steps = steps;
    if (recalculateSteps) recalculateStepData();
  }
  size_t* const getRodSteps() { return &rod_steps; }
  void commandMove(size_t destination) { commandMove((float)destination); }
  void commandMove(float destination) {
    rod_exact_command = destination;
  ...
  void fire(bool status) {
    if (status) {
      if (mode == OperationModes::Pulse ||
          rod_exact_command == 0.f) {  // functionallity on JSI TRIGA
        setEnabled(true);
      }
      if (mode == Pulse) {
        fireing = true;
  ...
  float getPositionAtPcm(float position_pcm) {
    ...
    if (position_pcm >= 1.f) {
      ...
      if (stepData[i] == position_pcm) {
        ...
        return (position_pcm - stepData[i - 1]) /
                   (stepData[i] - stepData[i - 1]) +
               i - 1;
      }
    }
  }
  ...
  void setParameter(size_t index, float value, bool recalculateSteps = true) {
  ...
  float getPCMat(float position) {
    ...
    if (floorP == ceilP) {
      ...
      pcmRelative = static_cast<double>(stepData[floorP] * (ceilP - position) +
                                        stepData[ceilP] * (position - floorP));
    }
    return (float)(pcmRelative * rod_worth);
  }
  float getCurrentPCM() { return getPCMat(rod_actual_position); }
  ...
  void setEnabled(bool status, bool force = false) {
    if (status) {
      if ((rod_actual_position == 0.f) && (rod_exact_position == 0.f) &&
              (mode == OperationModes::Manual) ||
          force) {
        enabled = true;
      }
    } else {
  ...
  Setting the speed to a non-positive number makes the control rods move
  infinitely fast. Default is 0.*/
  ...
  void refreshRod(double dt) {
    ...
    if (scramTime > 0.) {
      ...
      rod_actual_position =
          (float)(positionAtScram *
                  (1. - std::pow(timeSinceScram / scramTime, 2.)));
      ...
      // Move the rod
      if ((rod_exact_position != localCommand) &&
          (mode == OperationModes::Manual ||
           mode == OperationModes::Automatic ||
           mode == OperationModes::Pulse)) {
        if (rod_speed == 0.f && enabled) {
          ...
          if (!fireing) {
            ...
            if (localCommand > rod_exact_position) {
              moveRodToStep(
                  std::min(rod_exact_position + rod_move, localCommand));
            } else {
              moveRodToStep(
                  std::max(rod_exact_position - rod_move, localCommand));
            }
          }
        }
      }
  ...
  SimulationModes getSimulationMode() { return simulationMode; }
};
