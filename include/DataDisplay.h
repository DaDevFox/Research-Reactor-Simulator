...
class DataDisplay : public CustomWidget {
 protected:
  ...
  nanogui::Color mNumberColor;
  nanogui::Color pointerColor;
  float padding[4] = {25, 10, 0, 10};
  const std::string units_old[13] = {"a", "f", "p", "n", "micro", "m", "",
                                     "k", "M", "G", "T", "P",     "E"};
  const int units[13] = {0x61, 0x66, 0x70, 0x6E, 0xB5, 0x6D, 0,
                         0x6B, 0x4D, 0x47, 0x54, 0x50, 0x45};
  std::string unit;
 ...
 public:
  ...
  const nanogui::Color& numberColor() const { return mNumberColor; }
  void setNumberColor(const nanogui::Color& numberColor) {
    mNumberColor = numberColor;
  }

  const nanogui::Color& getPointerColor() const { return pointerColor; }
  void setPointerColor(const nanogui::Color& pointer_color) {
 ...
 public:
  virtual void draw(NVGcontext* ctx) override {
    ...
    if (abs(currentData) > absoluteLimit && absoluteLimit > 0.) {
      ...
      nvgText(ctx, xNow, yNow + yRange, formatNumber(currentData).c_str(),
              NULL);
    }
    ...
    if (pointerShown) {
      ...
      nvgBeginPath(ctx);
      nvgMoveTo(ctx, (float)mPos.x(),
                yNow + yRange - (numberFontSize + pointerSize) / 2.f);
      nvgLineTo(ctx, (float)mPos.x(),
                yNow + yRange - (numberFontSize - pointerSize) / 2.f);
      nvgLineTo(ctx, mPos.x() + (float)sqrt(3) * pointerSize / 2.f,
                yNow + yRange - numberFontSize / 2.f);
      nvgClosePath(ctx);
      nvgFill(ctx);
    }
  }
  ...
  DataDisplay(Widget* parent, const std::string& caption = "Untitled")
    ...
    mNumberColor = nanogui::Color(255, 255);
    pointerColor = nanogui::Color(255, 0, 0, 255);
    unit = "";
  }
  ...
  std::string formatNumber(double value) {
    ...
    switch (numberMode) {
      ...
      case DisplayMode::Scientific:
        ...
        ret += " " +
               ((order > -7 && order < 13 && order != 0)
                    ? std::string(utf8(units[order + 6]).data())
                    : "") +
               unit;
        return ret;
    }
