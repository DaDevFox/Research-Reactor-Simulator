...
class CustomGraph : public CustomWidget {
 public:
  CustomGraph(Widget* parent, size_t graphNumber,
  ...
  Plot* addPlot(size_t dataPoints, bool rewriting = false) {
    return (Plot*)addGraph(new Plot(dataPoints, rewriting));
  }
  ...
  BezierCurve* addBezierCurve() {
    return (BezierCurve*)addGraph(new BezierCurve());
  }
