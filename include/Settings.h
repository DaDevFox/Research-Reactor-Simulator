...
DEFAULT VALUES
=====================*/
constexpr auto GRAPH_DEFAULT_HEIGHT =
    0.47f;                                   // The relative height of the graph
...
// Rod speeds (steps/second), similar to measured values with a stopwatch at IJS
// Triga
...
class Settings {
 private:
  ...
  const double delayedGroupBetasDefault[6] = {
      0.23097e-3, 1.53278e-3, 1.3718e-3, 2.76451e-3, 0.80489e-3, 0.29396e-3};
  const double delayedGroupLambdasDefault[6] = {0.0124, 0.0305, 0.1115,
                                                0.301,  1.138,  3.01};
  const bool delayedGroupsEnabledDefault[6] = {true, true, true,
                                               true, true, true};
 ...
 public:
  struct ControlRodSettings {
   public:
    ...
    ControlRodSettings(size_t steps_, float rodWorth_, float rodSpeed_,
                       float rodCurve1, float rodCurve2) {
      rodSteps = steps_;
      rodWorth = rodWorth_;
      rodSpeed = rodSpeed_;
  ...
  struct SimulationSettings {
   public:
    ...
    SimulationSettings(float per, float ampl) {
      period = per;
      amplitude = ampl;
    }
  ...
  struct SquareWaveSettings : SimulationSettings {
   public:
    float xIndex[4] = {SQUARE_WAVE_START_UP_DEFAULT, SQUARE_WAVE_END_UP_DEFAULT,
                       SQUARE_WAVE_START_DOWN_DEFAULT,
                       SQUARE_WAVE_END_DOWN_DEFAULT};
  ...
  struct SawToothSettings : SimulationSettings {
   public:
    float xIndex[6] = {
        SAW_TOOTH_UP_START_DEFAULT,  SAW_TOOTH_UP_PEAK_DEFAULT,
        SAW_TOOTH_UP_END_DEFAULT,    SAW_TOOTH_DOWN_START_DEFAULT,
        SAW_TOOTH_DOWN_PEAK_DEFAULT, SAW_TOOTH_DOWN_END_DEFAULT};
  ...
  // float reactivityGraphLimits[2] = {CORE_EXCESS_REACTIVITY -
  // SHIM_ROD_WORTH_DEFAULT - REGULATORY_ROD_WORTH_DEFAULT -
  // SAFETY_ROD_WORTH_DEFAULT, CORE_EXCESS_REACTIVITY};	// 23, 24
  float reactivityGraphLimits[2] = {-1000, 1000};  // 23, 24
  float temperatureGraphLimits[2] = {TEMPERATURE_GRAPH_FROM_DEFAULT,
                                     TEMPERATURE_GRAPH_TO_DEFAULT};  // 25, 26
  ...
  SquareWaveSettings squareWave = SquareWaveSettings(
      SIMULATION_MODE_PERIOD_DEFAULT, SIMULATION_MODE_AMPLITUDE_DEFAULT);  // 0
  SineSettings sineMode = SineSettings(SIMULATION_MODE_PERIOD_DEFAULT,
                                       SIMULATION_MODE_AMPLITUDE_DEFAULT);  // 1
  SawToothSettings sawToothMode = SawToothSettings(
      SIMULATION_MODE_PERIOD_DEFAULT, SIMULATION_MODE_AMPLITUDE_DEFAULT);  // 2
  SquareWaveSettings ns_squareWave =
      SquareWaveSettings(SIMULATION_MODE_PERIOD_DEFAULT,
                         NEUTRON_SOURCE_MODULATION_INTENSITY_DEFAULT);  // 3
  SineSettings ns_sineMode =
      SineSettings(SIMULATION_MODE_PERIOD_DEFAULT,
                   NEUTRON_SOURCE_MODULATION_INTENSITY_DEFAULT);  // 4
  SawToothSettings ns_sawToothMode =
      SawToothSettings(SIMULATION_MODE_PERIOD_DEFAULT,
                       NEUTRON_SOURCE_MODULATION_INTENSITY_DEFAULT);  // 5
  ...
  double pulseLimits[2] = {PULSE_START_DEFAULT,
                           PULSE_END_AFTER_DEFAULT};  // 85, 86
  ...
  Settings() {
    rodSettings[0] =
        ControlRodSettings(SAFETY_ROD_STEPS_DEFAULT, SAFETY_ROD_WORTH_DEFAULT,
                           SAFETY_ROD_SPEED_DEFAULT, safetyRodCurveDefault[0],
                           safetyRodCurveDefault[1]);
    rodSettings[1] = ControlRodSettings(
        REGULATORY_ROD_STEPS_DEFAULT, REGULATORY_ROD_WORTH_DEFAULT,
        REGULATORY_ROD_SPEED_DEFAULT, regulatoryRodCurveDefault[0],
        regulatoryRodCurveDefault[1]);
    rodSettings[2] = ControlRodSettings(
        SHIM_ROD_STEPS_DEFAULT, SHIM_ROD_WORTH_DEFAULT, SHIM_ROD_SPEED_DEFAULT,
        shimRodCurveDefault[0], shimRodCurveDefault[1]);
  ...
  void saveArchive(std::string fileName) {
    ...
    archive(rodSettings, squareWave, ns_squareWave, sawToothMode,
            ns_sawToothMode, sineMode, ns_sineMode, betas, lambdas,
            groupsEnabled, waterCooling, neutronSourceInserted, graphSize,
            displayTime, reactivityGraphLimits, temperatureGraphLimits,
            curveFill, rodReactivityPlot, coreVolume, waterVolume,
            waterCoolingPower, neutronSourceActivity, promptNeutronLifetime,
            temperatureEffects, fissionPoisons, excessReactivity, vesselRadius,
            steadyCurrentPower, avoidPeriodScram, steadyGoalPower, steadyMargin,
            periodLimit, periodScram, powerLimit, powerScram, tempLimit,
            tempScram, waterTempLimit, waterTempScram, allRodsAtOnce,
            waterLevelLimit, waterLevelScram, pulseLimits, alpha0, alphaAtT1,
            alphaT1, alphaK, yAxisLog, automaticPulseScram, reactivityHardcore,
  ...
  void restoreArchive(std::string fileName) {
    ...
    iarchive(rodSettings, squareWave, ns_squareWave, sawToothMode,
             ns_sawToothMode, sineMode, ns_sineMode, betas, lambdas,
             groupsEnabled, waterCooling, neutronSourceInserted, graphSize,
             displayTime, reactivityGraphLimits, temperatureGraphLimits,
             curveFill, rodReactivityPlot, coreVolume, waterVolume,
             waterCoolingPower, neutronSourceActivity, promptNeutronLifetime,
             temperatureEffects, fissionPoisons, excessReactivity, vesselRadius,
             steadyCurrentPower, avoidPeriodScram, steadyGoalPower,
             steadyMargin, periodLimit, periodScram, powerLimit, powerScram,
             tempLimit, tempScram, waterTempLimit, waterTempScram,
             allRodsAtOnce, waterLevelLimit, waterLevelScram, pulseLimits,
             alpha0, alphaAtT1, alphaT1, alphaK, yAxisLog, automaticPulseScram,
             reactivityHardcore, squareWaveUsesRodSpeed);
  }
};
