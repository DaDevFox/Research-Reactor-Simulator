...
class ControlRodDisplay : public nanogui::Widget {
 public:
  ControlRodDisplay(Widget* parent) : Widget(parent) {};
  ~ControlRodDisplay() {};
  void setRod(int i, size_t* steps, float* actualPos, float* exactPos,
              bool* r_enabled) {
    rodSteps[i] = steps;
    rodActualPos[i] = actualPos;
    rodExactPos[i] = exactPos;
    rodEnabled[i] = r_enabled;
  }
