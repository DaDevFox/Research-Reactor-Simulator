...
struct PeriodicalMode {
 ...
 protected:
  ...
  bool isPaused =
      false;  // Boolean value indicating if the simulation is paused
 ...
 public:
  PeriodicalMode(float period_, float amplitude_) {
    period = period_;
  ...
  void setAmplitude(float steps) { amplitude = steps; }
  ...
  static const size_t num_points(SimulationModes sim_mode_) {
    return p_num((int)sim_mode_) + 2;
  }

  // Start the simulation from a desired point in time (0 to 1; 1 meaning the
  // period end)
  ...
  float getPeriodTime() { return fmod(time, period) / period; }
...
struct SquareWave : public PeriodicalMode {
 public:
  ...
  SquareWave(float period_, float amplitude_)
      : PeriodicalMode(period_, amplitude_) {
    sim_mode = SimulationModes::SquareWaveMode;
  }
  ...
  float getCurrentOffset(float t) override {
    ...
    if (rodSpeed) {
      if (t <= xIndex[0]) {
        ...
        return std::min(amplitude, (t - xIndex[0]) * getPeriod() * rodSpeed);
      } else if (t <= xIndex[2]) {
        return std::max(0.f, getRelOffsetOfPoint(2) * amplitude -
                                 (t - xIndex[1]) * getPeriod() * rodSpeed);
      } else if (t <= xIndex[3]) {
        return -std::min(amplitude, (t - xIndex[2]) * getPeriod() * rodSpeed);
      } else {
        return std::min(0.f, getRelOffsetOfPoint(6) * amplitude +
                                 (t - xIndex[3]) * getPeriod() * rodSpeed);
      }
    } else {
  ...
  float getRelTimeOfPoint(size_t p_i) override {
    if (rodSpeed) {
      if (p_i % 2) {
        ...
        return std::min((p_i == 7) ? 1.f : xIndex[(p_i + 1) / 2],
                        xIndex[p_i / 2] + stepTime);
      } else {
        return xIndex[p_i / 2];
      }
    } else {
  ...
  float getRelOffsetOfPoint(size_t p_i) override {
    if ((p_i % 4) % 3) {
      if (rodSpeed) {
        if (p_i < 3) {
          return std::min(
              (xIndex[1] - xIndex[0]) * getPeriod() * rodSpeed / amplitude,
              1.f);
        } else {
          return -std::min(
              (xIndex[3] - xIndex[2]) * getPeriod() * rodSpeed / amplitude,
              1.f);
        }
      } else {
...
struct Sine : public PeriodicalMode {
 public:
  ...
  Sine(float period_, float amplitude_) : PeriodicalMode(period_, amplitude_) {
    sim_mode = SimulationModes::SineMode;
  }
...
struct SawTooth : public PeriodicalMode {
 public:
  ...
  SawTooth(float period_, float amplitude_)
      : PeriodicalMode(period_, amplitude_) {
    sim_mode = SimulationModes::SawToothMode;
  }
