...
class FileDialog : public nanogui::Window {
 ...
 private:
  ...
  nanogui::Button* mDirUpButton;
  nanogui::Button* mOkButton;
  nanogui::Button* mCancelButton;
  nanogui::Screen* mScreen;
  nanogui::TextBox* mUrlBox;
  nanogui::TextBox* mFileBox;
  ...
  class ItemList : public nanogui::Widget {
   private:
   ...
   public:
    ItemList(nanogui::Widget* parent) : nanogui::Widget(parent) {}

    bool mouseButtonEvent(const nanogui::Vector2i& p, int button, bool down,
    ...
    void addItem(DirInfo itm) { displayData.push_back(itm); }
    ...
    DirInfo item(size_t index) { return displayData[index]; }
    ...
    nanogui::Vector2i preferredSize(NVGcontext* ctx) const override {
      nanogui::Vector2i ret = Widget::preferredSize(ctx);
      return nanogui::Vector2i(ret.x(), displayData.size() * itemHeight);
    }
  ...
  std::vector<DirInfo> openDir(const std::string& directory,
    ...
    if (directory.length()) {
      ...
      do {
        ...
        if (file_data.dwFileAttributes & FILE_ATTRIBUTE_NOT_CONTENT_INDEXED)
          continue;
 ...
 public:
  FileDialog(nanogui::Screen* parent, std::string title, std::string startDir,
             std::string filetypes, std::string confirmButtonText,
             bool isFolderGoal, bool save = false);
    ...
    mOkButton->setCallback([this]() {
      if (mCallback && canContinue()) {
        ...
        if (this->mSave && fileExists(fileN)) {
          nanogui::MessageDialog* msg = new nanogui::MessageDialog(
              this, nanogui::MessageDialog::Type::Warning, mTitle,
              "The file you chose already exists. Would you like to overwrite "
              "it?",
              "Yes", "No", true);
          msg->setPosition(
              nanogui::Vector2i((this->size().x() - msg->size().x()) / 2,
