...
public:
	FileDialog(nanogui::Screen* parent, std::string title, std::string startDir, std::string filetypes, std::string confirmButtonText, bool isFolderGoal, bool save = false);
		...
		mOkButton->setCallback([this]()
			{
				if (mCallback && canContinue())
				{
					...
					{
						nanogui::MessageDialog* msg = new nanogui::MessageDialog(this, nanogui::MessageDialog::Type::Warning, mTitle, "The file you chose already exists. Would you like to overwrite it?", "Yes", "No", true);
						msg->setPosition(nanogui::Vector2i((this->size().x() - msg->size().x()) / 2, (this->size().y() - msg->size().y()) / 2));
