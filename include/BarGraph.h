...
class BarGraph {
 ...
 public:
  ...
  void setRange(size_t fromIndex, size_t toIndex) {
    range[0] = fromIndex;
    range[1] = toIndex;
  }
  void setAutoscaleRange(size_t fromIndex, size_t toIndex) {
    autoscaleRange[0] = fromIndex;
    autoscaleRange[1] = toIndex;
  }
  ...
  void updateAutoscale() {
    nSum = 0;
    for (size_t i = autoscaleRange[0]; i <= autoscaleRange[1]; i++) {
      nSum += data->back()[i];
    }
  }
