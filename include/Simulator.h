...
constexpr auto WATER_ABSORBTION =
    -0.3635;  // fluks(h) = phi0 * e^(this * h) [cm^-1]
// Measured at IJS triga in 2016
constexpr auto WATER_HEIGHT =
    5.;  // height of water level above (point)reactor [m]
...
class Simulator {
 private:
  ...
  double fuel_density =
      6;  // g/cm3, from http://www.rcp.ijs.si/ric/description-s.html
  ...
  */
  ...
  // From the "new power method", we multiply the k*S/C =0.022 kWh/K by C=19.6
  // kWh/K
 ...
 public:
  ...
  enum TemperatureMode : std::uint8_t { Asymptotic = 0, FH = 1 };
  ...
  // Pleaceholder to calculate delayed group fractions outside the GUI app in
  // the future
  ...
  // Toggles debug mode
  void setDebugMode(bool& value) { debug_mode = value; }
  ...
  const bool& getFissionPoisoningEffectsEnabled() {
    return fissionPoisoning_effects;
  }
  ...
  const size_t getNextIndex() const { return iterations_total % dataPoints; }
  ...
  /* Gets or sets the macroscopic cross section cross section(in 1/m) for a
  fission reaction to occour. Default is 2,81111/m.*/
  ...
  /* Gets or sets the fraction of total neutrons that are part of the delayed
  groups(6 groups). Defaults
  are: 2.09999998E-04, 1.30600005E-03, 1.18999998E-03,
  ...
  double getCurrentFlux() {
    return state_vector_[0][getCurrentIndex()] * t_neutron_speed /
           (getReactorCoreVolume());
  }
  ...
  /* Gets or sets the reactivity of the reactor core with no control rods
  inserted. Default is 2000 pcm.*/
  ...
  /* Gets or sets the amplitude of the neutron source simulation modes(in
  neutrons per second). Default is 10^5 neutrons per second.*/
  ...
  /* Gets or sets the time in seconds after which values older than x seconds
  will be deleted. Setting this to 0 disables the deleting of old data values.
  ...
  // Returns the number of data samples in the last <see cref="LoopFinished"/>
  // event.
  ...
  // Returns the number of calculations performed since the start of the
  // simulation.
  ...
  // Returns the power at which the automatic mode should keep the reactor
  // stable
  ...
  // If true, the automatic mode will keep the reactor steady at the current
  // power
  void setKeepCurrentPower(bool value) { keepCurrentPower = value; }

  // Returns true if automatic mode avoids triggering a period SCRAM when moving
  // the control rod
  bool& getAutomaticAvoidPeriodScram() { return avoidPeriodScram; }
  // If true, the automatic mode will avoid triggering a period SCRAM when
  // moving the control rod
  void setAutomaticAvoidPeriodScram(bool value) { avoidPeriodScram = value; }

  // Returns the raw fraction (power-steadyPower)/steadyPower at which the
  // control rod is automatically moved
  double getAutomaticDeviation() { return steadyDeviation; }
  // Sets the raw fraction (power-steadyPower)/steadyPower at which the control
  // rod is automatically moved
  ...
  Keep in mind this does not push values to any other deques than the state
  vector
  */
 ...
 private:
  ...
  void advanceSourceTime(double dt) {
    if (source_mode != SimulationModes::None)
      getSourceModeClass(source_mode)->handleAddTime((float)dt);
  };
  ...
  static void arraySum(double* sum_array1, double* sum_array2,
                       bool posOnly = false);
  ...
  static std::string formatTime(double t) {
    ...
    for (int i = 0; i < 4; i++) {
      if (i == 3) {
        ...
        ret[i] = ((time[i] < 10) ? ("0" + to_string(time[i]))
                                 : (to_string(time[i])));
      }
    }
