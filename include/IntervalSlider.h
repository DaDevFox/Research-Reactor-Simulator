...
class IntervalSlider : public nanogui::Widget {
 public:
  ...
  void setHighlightColor(const nanogui::Color& highlightColor) {
    mHighlightColor = highlightColor;
  }

  std::function<void(float)> callback(int i) const { return mCallback[i]; }
  void setCallback(int i, const std::function<void(float)>& callback) {
    mCallback[i] = callback;
  }

  std::function<void(float)> finalCallback(int i) const {
    return mFinalCallback[i];
  }
  void setFinalCallback(int i, const std::function<void(float)>& callback) {
    mFinalCallback[i] = callback;
  }
