...
class GraphElement {
 public:
  enum HorizontalAxisLocation { Bottom, Top };
  enum AxisLocation { Left, Right };
  enum FormattingMode { Exponential, Normal };
  enum GraphType {
    None,
    PlotDeque,
    Bezier,
 ...
 protected:
  ...
  HorizontalAxisLocation horizontalAxisPosition =
      HorizontalAxisLocation::Bottom;
 ...
 public:
  ...
  void setLimitOverride(size_t limitIndex, std::string value) {
    mOverrideLimitLabels[limitIndex] = value;
  }
  ...
  const bool& getHorizontalMainLineShown() const {
    return drawHorizontalMainAxis;
  }
  void setHorizontalMainLineShown(bool value) {
    drawHorizontalMainAxis = value;
  }
  ...
  const bool& getHorizontalPointerShown() const {
    return horizonralPointerShown;
  }
  ...
  void setHorizontalPointerPosition(float value) {
    horizontalPointerPosition = value;
  }
  const float& getHorizontalPointerPosition() {
    return horizontalPointerPosition;
  }
  ...
  void setHorizontalPointerColor(Color value) {
    horizontalPointerColor = value;
  }
  const Color& getHorizontalPointerColor() const {
    return horizontalPointerColor;
  }
  ...
  void setHorizontalMinorTickNumber(size_t value) {
    minorTickNumberHorizontal = value;
  }
  const size_t& getHorizontalMinorTickNumber() const {
    return minorTickNumberHorizontal;
  }
  ...
  void setHorizontalMajorTickNumber(size_t value) {
    majorTickNumberHorizontal = value;
  }
  const size_t& getHorizontalMajorTickNumber() const {
    return majorTickNumberHorizontal;
  }
  ...
  void setHorizontalTextOffset(float value) {
    horizontalAxisTextOffset = value;
  }
  const float& getHorizontalTextOffset() const {
    return horizontalAxisTextOffset;
  }
  void setHorizontalAxisPosition(HorizontalAxisLocation value) {
    horizontalAxisPosition = value;
  }
  const HorizontalAxisLocation& getHorizontalAxisPosition() const {
  ...
  void setLimitHorizontalMultiplier(double value) {
  ...
  void setHorizontalNumberFormatMode(FormattingMode value) {
    horizontalNumberFormat = value;
  }
  const FormattingMode& getHorizontalNumberFormatMode() const {
    return horizontalNumberFormat;
  }
  ...
  void setLimits(double minX, double maxX, double minY, double maxY) {
    ...
    for (int i = 0; i < 4; i++)
      mLogLimits[i] = mLimits[i] ? log10(mLimits[i]) : -10.;
...
class Plot : public GraphElement {
 ...
 public:
  Plot(const size_t arraySize, bool rewriting = false) : mArraySize(arraySize) {
    mRewriting = rewriting;
  };
  ...
  void setPlotRange(size_t from, size_t to) {
    plotRange[0] = from;
    plotRange[1] = (to < from && mRewriting) ? to + mArraySize : to;
  }
  ...
  void setValueComputing(std::function<void(double*, const size_t)> computing) {
    mValueComputing = computing;
  }
  std::function<void(double*, const size_t)> valueComputing() {
    return mValueComputing;
  }
 ...
 public:
  ...
  void setXdataLin(long indexOffset) {
    if (indexOffset >= 0) start = indexOffset;
  }
  void setXdata(double* x_axis) { xValues = x_axis; }
  void setYdata(double* y_axis) {
    yValues_dbl = y_axis;
    type = 2;
  }
  void setYdata(float* y_axis) {
    yValues_float = y_axis;
    type = 1;
  }
  ...
  double getXat(size_t i, bool normalize = true) {
    if (start >= 0L) {
      if (normalize) {
        return ((double)i - plotRange[0] - start) /
  ...
  double getYat(size_t i, bool normalize = true) {
    if (mRewriting) i = i % mArraySize;
    // if (i >= mArraySize) throw new std::exception("Out of bounds - Plot
    // deque");
